...
| **Input trust (optional)** | `KERNEL_SEMANTIC_CHAT_GATE`, `KERNEL_SEMANTIC_CHAT_EMBED_MODEL`, `KERNEL_SEMANTIC_CHAT_SIM_THRESHOLD` | Ollama `/api/embeddings` layer before substring MalAbs when gate is on; **no effect** when off; degrades to substring-only if Ollama/embed model unavailable ([`LLM_STACK_OLLAMA_VS_HF.md`](LLM_STACK_OLLAMA_VS_HF.md), ADR 0003). |
