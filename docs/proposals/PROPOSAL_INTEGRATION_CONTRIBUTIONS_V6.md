...
Incorporate only what **adds measurable new state** or **bounded proactive behavior**, without duplicating poles, PAD, σ, episodic narrative, or Psi Sleep/DAO as they already exist.

**Principles:**

1. **MalAbs and buffer** remain anchors; no phase modifies them in production.
2. **Nothing new** in the critical ethical veto path without regression tests.
3. Each phase = **bounded increment** + **falsifiable criteria** (metrics in simulation or logs).
...
5. **Buffer** — context-activated principles.  
6. **Bayes** — expected action selection.  
7. **Poles** — `TripartiteMoral` (scores, verdict, multipolar narrative).  
8. **Sigmoid will** — D_fast / gray_zone / etc. mode.  
9. **PAD + archetypes** — affective projection **post-decision**; does not govern the veto.  
10. **Narrative memory** — episode (with `affect_pad` / `affect_weights` if applicable); **`NarrativeIdentityTracker`** updates self-state only when registering an episode (`src/modules/narrative_identity.py`).  
11. **Weakness** — emotional load / weak episode registration.  
12. **Algorithmic forgiveness** — registration per episode.  
13. **DAO** — auditing; solidarity alerts if high risk.
...
| Module | File | Summary function |
|--------|---------|-------------------|
| MalAbs | `absolute_evil.py` | Action veto + conservative `evaluate_chat_text()`. |
| Constitution | `buffer.py` | Context-preloaded principles. |
| Impact | `bayesian_engine.py` | Action expectations; pruning. |
| Poles | `ethical_poles.py` | Dynamic multipolar → `TripartiteMoral`. |
| Will | `sigmoid_will.py` | Decision mode by impact/uncertainty. |
| Body | `sympathetic.py` | σ, energy, sympathetic/parasympathetic mode. |
| Long memory | `narrative.py` | Episodes, morals, σ, optional PAD in episode. |
| Social | `uchi_soto.py` | Trust circles. |
| Attribution | `locus.py` | Locus of control. |
| Night | `psi_sleep.py` | Auditing, loose recalibrations. |
...
| Humanization | `weakness_pole.py` | Narrative imperfection without changing the decision. |
...
| Drives (post-sleep) | `drive_arbiter.py` | Advisory discrete intentions after backup. |
| Narrative identity | `narrative_identity.py` | EMA over episodes; LLM context + chat JSON. |
...
- **Decisional tension:** poles + Bayesian uncertainty + will mode.  
- **Body / alert:** σ, Uchi-Soto.  
- **Tone / affect:** post-decision PAD.  
- **History:** episodes + Psi Sleep.  
- **Dialogue:** `WorkingMemory` + chat + WebSocket.  

**Covered in v6 repo:** second-order reflection (**Phase 1**), salience (**Phase 2**), advisory drives (**Phase 3**), narrative self (**Phase 4**), compact monologue (**Phase 5**). What remains **out of scope** by default is self-modification of operative ethics (see excluded section).
...
**Where:** in `EthicalKernel.process`, after final mode and action selection, **before** PAD; field `KernelDecision.reflection`. Exposed in `format_decision` and `chat_server` JSON. `reflection_to_llm_context()` feeds `LLMModule.communicate(..., reflection_context=...)` in `process_natural` and `process_chat_turn`.

**Use:** explainability, telemetry, and voice nuance (without changing the decision). **Does not** modify the action or MalAbs.
...
**What:** `src/modules/salience_map.py` — `SalienceSnapshot` with `weights` normalized across four axes (`risk`, `social`, `body`, `ethical_conflict`), `dominant_focus`, `raw_scores`. `social` combines hostility, `caution_level`, and dialectic; `body` uses σ; `ethical_conflict` uses `reflection.strain_index`.
...
**Evolutionary:** weighting locus/Bayes (±ε) only with a regression battery — **not** done yet.

---

### Phase 3 — `DriveArbiter` (bounded teleology) — **implemented**

**What:** `src/modules/drive_arbiter.py`. Evaluation **after** Psi Sleep and immortality (`EthicalKernel.execute_sleep`); also exposed in WebSocket JSON as `drive_intents` (advisory view on each turn). Emits up to four `DriveIntent` with `suggest`, `reason`, `priority`.

**Does not** execute physical actions without a policy/DAO layer; **does not** invent MalAbs-fragile actions.

**Criterion:** traceability; bounded and priority-ordered list.

---

### Phase 4 — `NarrativeIdentity` (persistent self) — **implemented**

**What:** `NarrativeIdentityTracker` in `NarrativeMemory.identity`; `update_from_episode` only when registering an episode. `ascription_line()` / `to_llm_context()` feed `LLMModule.communicate(..., identity_context=...)` and the chat JSON (`identity` with leans + `ascription`).

**Does not** replace episodes; **does not** modify pole weights.

---

### Phase 5 — `InternalMonologue` (structured record) — **implemented**

**What:** `compose_monologue_line(decision, episode_id)` in `internal_monologue.py` — compact `[MONO]` line for `format_decision` and `monologue` field in `chat_server` JSON when there is a `KernelDecision`.
