...
**Scope and tone:** this tree mixes implemented contracts with exploratory and aspirational work; when in doubt, read [ASPIRATIONAL_DISCLAIMER.md](ASPIRATIONAL_DISCLAIMER.md) before taking any proposal as a strict commitment.
...
| Document | Summary |
|----------|---------|
| [`../adr/0012-bayesian-weight-inference-ethical-mixture-scorer.md`](../adr/0012-bayesian-weight-inference-ethical-mixture-scorer.md) | Optional BMA (L1), feedback Dirichlet updates (L2), context buckets (L3); softmax / importance-sampling likelihood; env vars, tests, and module map. |
| [`PROPOSAL_ETHICAL_CORE_LOGIC_EVOLUTION.md`](PROPOSAL_ETHICAL_CORE_LOGIC_EVOLUTION.md) | Psi Sleep v2 threads, semantic MalAbs gate path, mixture / ledger evolution notes. |
| [`LANDING_DECOUPLING_AND_SUPPORT.md`](LANDING_DECOUPLING_AND_SUPPORT.md) | How the lightweight kernel line relates to landing + publication payloads. |
| [`STRATEGY_AND_ROADMAP.md`](STRATEGY_AND_ROADMAP.md) | P0–P3 priorities, robustness → epistemology → demo ordering. |

| Area | What changed |
|------|----------------|
| **Docs** | Single **`docs/proposals/`** tree; repo links point here. ADRs live in [`docs/adr/`](../adr/README.md). Root [`BIBLIOGRAPHY.md`](../../BIBLIOGRAPHY.md) and `landing/` ship on branch **`main-whit-landing`**. |
