...
- **Delivered (baseline):** Introduced a minimal `SensorAdapter` seam with runtime adapter (`LayeredSensorAdapter`) and deterministic doubles (`NullSensorAdapter`, `StaticSensorAdapter`, `SequenceSensorAdapter`) so perception-facing code stays transport/vendor-agnostic while preserving existing fixture/preset/client merge semantics.
- **Implementation target:**
  - Keep the kernel contract stable (`SensorSnapshot`), and evolve adapter implementations independently.
  - Use deterministic adapters in tests instead of ad-hoc payload wiring.
- **Evidence links (start):**
  - [`PROPOSAL_SITUATED_ORGANISM_V8.md`](PROPOSAL_SITUATED_ORGANISM_V8.md)
  - [`src/modules/sensor_adapters.py`](../../src/modules/sensor_adapters.py)
  - [`src/chat_server.py`](../../src/chat_server.py)
  - [`tests/test_sensor_adapters.py`](../../tests/test_sensor_adapters.py)

#### SP-P1-02 — Sensor fusion input normalization profile *(landed baseline)*
...
- **Target branch:** `master-Cursor`
- **Delivered (baseline):** Added canonical normalization profile for incoming sensor payloads before they influence perception/risk signals (`battery_level` percent -> unit interval, ambient noise dB -> unit interval, bounded jerk scaling, strict bool parsing for `backup_just_completed`, unknown key drop).
- **Failure handling / fallback posture:** normalization now reports and enforces explicit modes (`missing_sensor_payload`, `noise_only_payload`, `partial_payload`, `clean_payload`) with quality tags (`none`, `partial`, `full`).
- **Implementation target:**
  - Keep profile constants explicit and test-locked (`DEFAULT_DB_MAX`, `DEFAULT_ACCEL_JERK_MAX`, `DEFAULT_SENSOR_KEYS`).
  - Extend with operator-facing telemetry in a future slice without changing the kernel `SensorSnapshot` contract.
- **Evidence links (start):**
  - [`PROPOSAL_SITUATED_ORGANISM_V8.md`](PROPOSAL_SITUATED_ORGANISM_V8.md)
  - [`src/modules/sensor_normalization_profile.py`](../../src/modules/sensor_normalization_profile.py)
  - [`src/modules/sensor_adapters.py`](../../src/modules/sensor_adapters.py)
  - [`tests/test_sensor_normalization_profile.py`](../../tests/test_sensor_normalization_profile.py)
