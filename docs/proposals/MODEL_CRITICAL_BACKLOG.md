...
| 2 | **Unified LLM degradation posture** | **Operator surface closed (G-04):** touchpoint keys, legacy keys, optional `KERNEL_LLM_GLOBAL_DEFAULT_POLICY`, monologue policies, `GET /health` → `llm_degradation` — [PROPOSAL_LLM_INTEGRATION_TRACK.md](PROPOSAL_LLM_INTEGRATION_TRACK.md). **Still future / cross-cutting:** embedding transport patterns vs chat completion ([PROPOSAL_LLM_TOUCHPOINT_DEGRADATION_MATRIX.md](PROPOSAL_LLM_TOUCHPOINT_DEGRADATION_MATRIX.md)); **renaming every legacy knob** under one prefix remains deferred ([WEAKNESSES_AND_BOTTLENECKS.md](../WEAKNESSES_AND_BOTTLENECKS.md) §3). | [WEAKNESSES_AND_BOTTLENECKS.md](../WEAKNESSES_AND_BOTTLENECKS.md) §3; [PROPOSAL_PERCEPTION_BACKEND_DEGRADATION_POLICY.md](PROPOSAL_PERCEPTION_BACKEND_DEGRADATION_POLICY.md); [PROPOSAL_LLM_VERBAL_DEGRADATION_POLICY.md](PROPOSAL_LLM_VERBAL_DEGRADATION_POLICY.md) |
| 3 | **Core decision chain + packaging boundary** | Operators need a clear “what ships as the kernel” story; avoids implied guarantees outside the tested entry path. | [CRITIQUE_ROADMAP_ISSUES.md](CRITIQUE_ROADMAP_ISSUES.md) Issue **#4**; [CORE_DECISION_CHAIN.md](CORE_DECISION_CHAIN.md); [ADR 0001](../adr/0001-packaging-core-boundary.md); drift guard [`tests/test_core_packaging_boundary_docs.py`](../../tests/test_core_packaging_boundary_docs.py) |
...
- **`KERNEL_*` combinatorics (Issue 7):** **Documented + tested** — [`KERNEL_ENV_POLICY.md`](KERNEL_ENV_POLICY.md) implementation status; [`tests/test_env_policy.py`](../../tests/test_env_policy.py). Issue #7 does **not** remove the need for a future **legacy prefix rename** ([`WEAKNESSES_AND_BOTTLENECKS.md`](../WEAKNESSES_AND_BOTTLENECKS.md) §3); **G-04** already delivered the unified operator index + global default fallback.
