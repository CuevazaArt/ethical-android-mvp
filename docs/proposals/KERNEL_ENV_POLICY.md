...
| **Perception / sensors** | `KERNEL_SENSOR_*`, `KERNEL_MULTIMODAL_*`, `KERNEL_VITALITY_*`, `KERNEL_PERCEPTION_UNCERTAINTY_DELIB`, `KERNEL_PERCEPTION_UNCERTAINTY_MIN` | Bounds and tuning — **heuristic** ([INPUT_TRUST_THREAT_MODEL.md](INPUT_TRUST_THREAT_MODEL.md)). When `KERNEL_PERCEPTION_UNCERTAINTY_DELIB=1`, chat / `process_natural` pass perception coercion **uncertainty** into `EthicalKernel.process`; if it is **≥** `KERNEL_PERCEPTION_UNCERTAINTY_MIN` (default `0.35`), **`D_fast` → `D_delib`** (optional hardening spike; default **off**). |
