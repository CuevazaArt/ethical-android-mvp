...
| **Governance / hub** | `KERNEL_MORAL_HUB_*`, `KERNEL_DEONTIC_GATE`, `KERNEL_LOCAL_SOVEREIGNTY`, `KERNEL_JUDICIAL_*`, `KERNEL_DAO_INTEGRITY_AUDIT_WS`, `KERNEL_LAN_GOVERNANCE_MERGE_WS`, `KERNEL_AUDIT_SIDECAR_PATH` | Demos, audits, mock court — **not** on-chain consensus ([GOVERNANCE_MOCKDAO_AND_L0.md](GOVERNANCE_MOCKDAO_AND_L0.md)). `KERNEL_LOCAL_SOVEREIGNTY=0` disables L0 heuristic scan on JSON calibration payloads (`local_sovereignty.py`). **`KERNEL_DAO_INTEGRITY_AUDIT_WS`:** WebSocket `integrity_alert`. **`KERNEL_LAN_GOVERNANCE_MERGE_WS`:** WebSocket `lan_governance_integrity_batch` (requires integrity audit on), `lan_governance_dao_batch` (requires DAO vote on), `lan_governance_judicial_batch` (requires `KERNEL_JUDICIAL_ESCALATION=1`), and `lan_governance_mock_court_batch` (requires `KERNEL_JUDICIAL_MOCK_COURT=1`; see `moral_hub.py` + `chat_server.py`). **`KERNEL_AUDIT_SIDECAR_PATH`:** append-only JSONL mirror of `MockDAO.register_audit` rows for operator log separation (not a signed service). |
...
| **Mixture weights (episodic nudge)** | `KERNEL_BAYESIAN_EMPIRICAL_WEIGHTS` | When `1`, nudges discrete mixture weights from recent `NarrativeMemory` episodes **in the same context** before scoring; default **off** (fixed mixture); **not** full Bayesian inference ([ADR 0009](../adr/0009-ethical-mixture-scorer-naming.md)). |
