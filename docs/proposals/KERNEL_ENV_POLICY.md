...
| **Input trust (optional)** | `KERNEL_SEMANTIC_CHAT_GATE`, `KERNEL_SEMANTIC_CHAT_EMBED_MODEL`, `KERNEL_SEMANTIC_CHAT_SIM_BLOCK_THRESHOLD`, `KERNEL_SEMANTIC_CHAT_SIM_ALLOW_THRESHOLD` (legacy `KERNEL_SEMANTIC_CHAT_SIM_THRESHOLD` → block), `KERNEL_SEMANTIC_CHAT_LLM_ARBITER` | Lexical MalAbs first, then Ollama embeddings, then optional LLM arbiter ([`MALABS_SEMANTIC_LAYERS.md`](MALABS_SEMANTIC_LAYERS.md), ADR 0003). **No effect** when gate off; if Ollama down, lexical only. |
