...
| **Input trust (optional)** | `KERNEL_SEMANTIC_CHAT_GATE`, `KERNEL_SEMANTIC_EMBED_HASH_FALLBACK`, `KERNEL_SEMANTIC_CHAT_EMBED_MODEL`, `KERNEL_SEMANTIC_CHAT_SIM_*`, `KERNEL_SEMANTIC_CHAT_LLM_ARBITER` (plus transport vars in `semantic_embedding_client`) | Lexical MalAbs first, then embeddings, then optional LLM arbiter ([`MALABS_SEMANTIC_LAYERS.md`](MALABS_SEMANTIC_LAYERS.md), ADR 0003). **Hash fallback** keeps the tier on without Ollama (nominal LAN/hub profiles set gate + fallback). If gate on and embeddings unavailable **without** fallback, semantic tier defers → lexical only. |
