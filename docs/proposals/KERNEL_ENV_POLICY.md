...
| **Chat async bridge** | `KERNEL_CHAT_TURN_TIMEOUT`, `KERNEL_CHAT_THREADPOOL_WORKERS` | Per-turn **async** deadline (JSON `chat_turn_timeout`); optional dedicated thread pool for `process_chat_turn` offload ([ADR 0002](../adr/0002-async-orchestration-future.md), `real_time_bridge.py`). |
