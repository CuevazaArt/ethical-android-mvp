...
|--------|-------------------|------|
| **Chat JSON telemetry** | `KERNEL_CHAT_INCLUDE_*` (incl. **`KERNEL_CHAT_INCLUDE_TRANSPARENCY_S10`** — optional `transparency_s10`), `KERNEL_CHAT_EXPOSE_MONOLOGUE`, prefetch / charm UX flags in [`ENV_VAR_CATALOG.md`](../ENV_VAR_CATALOG.md) | Omit or include **UX fields** in WebSocket payloads — **no** ethical veto. Normative boundary vs MalAbs / `KernelDecision`: [ADR 0018](../adr/0018-presentation-tier-vs-ethical-core.md). |
...
| **Nomad / identity** | `KERNEL_NOMAD_*` (incl. **`KERNEL_NOMAD_TELEMETRY_VITALITY`** — merge last LAN `telemetry` into vitality when fields missing), `KERNEL_CHAT_INCLUDE_NOMAD_IDENTITY` | Lab simulation + JSON — **stubs** where noted in docs; vitality merge is real path when Nomad bridge is active ([`ENV_VAR_CATALOG.md`](../ENV_VAR_CATALOG.md) §11). |
...
| **Fleet stub (lab)** | `KERNEL_FLEET_STUB` | Enables optional use of fleet peer stub digest helpers in tooling — **no** P2P stack, **no** kernel change. (Legacy alias retired V2.159; see `DEPRECATION_ROADMAP.md §3`.) |
