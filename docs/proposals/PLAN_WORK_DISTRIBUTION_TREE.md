...
>
> **Fase 15 (L0) — Sustracción y consolidación [IN_PROGRESS — Swarm L2]:** retirar ruido operativo (logs de depuración, `except` silenciosos), alinear con **CI (GitHub Actions)** como verdad de calidad, sin asumir `pytest` local limpio. Entregas puntuales: `CHANGELOG.md` + `python scripts/swarm_sync.py --msg '...'`. **Tarea 15.0.1 [DONE — Cursor]:** reparar indentación rota en `EthicalKernel.seek_internal_purpose` / `register_turn_feedback` (import y colección de tests); acotar energía en `emit_proactive_pulse_if_idle` (finito, ``[0,1]``); `py_compile src/kernel.py`; `src/MANIFEST.json` actualizado.
...
- **Bloque S.2: Calibración Termo-Visual Continua [DONE]**
  - Tarea S.2.1: Refinar las interrupciones del `VitalityAssessment` (ej. alertas de calor del dispositivo) utilizando la telemetría real transmitida por el *Nomad Bridge*. **[DONE — Abril 2026, Cursor]:** Nomad backfill vía ``merge_nomad_telemetry_into_snapshot`` / ``KERNEL_NOMAD_TELEMETRY_VITALITY``; ``assess_vitality`` usa ``_finite_clamp01`` / ``_finite_jerk`` / ``_finite_celsius``; banda ``KERNEL_VITALITY_WARNING_TEMP`` + histéresis ``KERNEL_VITALITY_THERMAL_HYSTERESIS`` / ``KERNEL_VITALITY_THERMAL_HYSTERESIS_C``; campo ``thermal_elevated`` en ``VitalityAssessment``; ``KERNEL_VITALITY_IMPACT_JERK_THRESHOLD``; alias Nomad → ``core_temperature`` (``skin_temperature``, ``battery_temp_c``, …); ``apply_nomad_telemetry_if_enabled`` en ``PerceptiveLobe._chat_assess_sensor_stack`` + ``peek_latest_telemetry``; ``vitality_communication_hint`` con ``trust_level`` finito; tests [`tests/test_vitality.py`](../../tests/test_vitality.py); **CI** ``.github/workflows/ci.yml`` (``pytest tests/`` completo).
