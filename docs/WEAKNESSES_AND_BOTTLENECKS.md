...
**Gap (partial mitigation).** Perception now exposes ``KERNEL_PERCEPTION_BACKEND_POLICY`` (``template_local`` / ``fast_fail`` / ``session_banner``) with structured ``coercion_report`` diagnostics — see [`PROPOSAL_PERCEPTION_BACKEND_DEGRADATION_POLICY.md`](proposals/PROPOSAL_PERCEPTION_BACKEND_DEGRADATION_POLICY.md). **Communicate / narrate** now have ``KERNEL_VERBAL_LLM_BACKEND_POLICY`` and chat ``verbal_llm_observability`` — [`PROPOSAL_LLM_VERBAL_DEGRADATION_POLICY.md`](proposals/PROPOSAL_LLM_VERBAL_DEGRADATION_POLICY.md). **Optional unified fallback:** ``KERNEL_LLM_GLOBAL_DEFAULT_POLICY`` applies after legacy keys and before built-in defaults, but **only** where the value is valid for that resolver (perception vs verbal vs monologue differ). A **single prefix** for every LLM-related knob and **embedding** transport unified with completion HTTP remain aspirational — see the matrix.
