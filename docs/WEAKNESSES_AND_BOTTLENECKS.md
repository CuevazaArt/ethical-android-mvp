...
**Gap (partial mitigation).** Perception now exposes ``KERNEL_PERCEPTION_BACKEND_POLICY`` (``template_local`` / ``fast_fail`` / ``session_banner``) with structured ``coercion_report`` diagnostics — see [`PROPOSAL_PERCEPTION_BACKEND_DEGRADATION_POLICY.md`](proposals/PROPOSAL_PERCEPTION_BACKEND_DEGRADATION_POLICY.md). **Communicate / narrate** now have ``KERNEL_VERBAL_LLM_BACKEND_POLICY`` and chat ``verbal_llm_observability`` — [`PROPOSAL_LLM_VERBAL_DEGRADATION_POLICY.md`](proposals/PROPOSAL_LLM_VERBAL_DEGRADATION_POLICY.md). **Unified operator surface (G-04):** touchpoint keys, legacy keys, and optional ``KERNEL_LLM_GLOBAL_DEFAULT_POLICY`` are documented in [`KERNEL_ENV_POLICY.md`](proposals/KERNEL_ENV_POLICY.md) and resolved in [`llm_touchpoint_policies.py`](../src/modules/llm_touchpoint_policies.py) / related policy modules; invalid legacy values fall through to the global default when valid for that resolver ([`PROPOSAL_LLM_TOUCHPOINT_DEGRADATION_MATRIX.md`](proposals/PROPOSAL_LLM_TOUCHPOINT_DEGRADATION_MATRIX.md)). A **single prefix** for every historical knob and **embedding** transport unified with completion HTTP remain aspirational — see the matrix.
