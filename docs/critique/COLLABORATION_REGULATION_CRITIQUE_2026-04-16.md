...
| **R-3** | Define **minimum pulse triggers** (examples): before any PR touching `CHANGELOG.md` top section; before any PR touching `src/kernel.py`; or weekly for active hubs. Keep them as **triggers**, not arbitrary calendars. | **Done:** table in [`MERGE_AND_HUB_DECISION_TREE.md`](../collaboration/MERGE_AND_HUB_DECISION_TREE.md) § *Minimum Integration Pulse triggers*. |
| **R-4** | When direct push to `master-<team>` is used, require **PR within 24h** or a linked issue summarizing commits (maintainer policy). | **Done (soft policy):** [`MERGE_AND_HUB_DECISION_TREE.md`](../collaboration/MERGE_AND_HUB_DECISION_TREE.md) § *Direct push to master-<team> vs PR*. |
| **R-5** | In integration merges, standardize not only `merge(sync):` but **`merge(integration):`** when merging **from** Antigravity staging into a team hub after L1 stabilization—distinguishes “peer catch-up” from “promotion prep.” | **Done:** [`MERGE_AND_HUB_DECISION_TREE.md`](../collaboration/MERGE_AND_HUB_DECISION_TREE.md) § *Merge commit message convention*. |
