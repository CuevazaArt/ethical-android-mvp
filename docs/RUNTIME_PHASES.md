...
| **2.3 Encryption (planned, not MVP)** | For deployments with sensitive on-disk data, an **encryption layer** on the snapshot (JSON/SQLite) or columns; typical Python choice is **`cryptography`** with **keys outside the repo** (secret manager / env). | **Not implemented yet** — MVP stores plaintext on purpose. Does not replace OS permissions or access control; document threat model when added. |
