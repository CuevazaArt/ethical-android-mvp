...
5. **Make sure the tests pass**: `pytest tests/ -v` (full suite; the `quality` job runs the same on Ubuntu for Python 3.11, 3.12, and 3.13. The `windows-smoke` job runs L1 invariants plus Ruff, Mypy, and a **targeted** pytest pair for profiles/env on win32 — see `.github/workflows/ci.yml`)
