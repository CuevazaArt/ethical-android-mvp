...
- **Bloque 16.0.8 — Tri-Lobe bus memory in Cursor integration gate:** `scripts/eval/run_cursor_integration_gate.py` and `docs/collaboration/CURSOR_CROSS_TEAM_INTEGRATION_GATE.md` list `tests/test_kernel_tri_lobe_bus_memory.py` (PLAN **26.0.1**, MemoryLobe ↔ `EVENT_KERNEL_*` amnesia / episode registration; anti-drift con Tri-Lobe). **Validation: GitHub Actions** (`pytest tests/` + gate).
