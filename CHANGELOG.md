...
### Cursor Team Updates
- **Bloque 0.1.10 / anti-shadow — `src.kernel` reexports vs `kernel_utils`:** `tests/test_kernel_utils.py` — `test_kernel_module_reexports_match_kernel_utils` uses object identity so `kernel_dao_as_mock` and `kernel_mixture_scorer` in `src/kernel` remain aliases of `src/kernel_utils` (prevents a future duplicate `def` in `kernel.py` from shadowing imports). Complements **8.1.30**. **Validation: GitHub Actions** (`pytest tests/`). `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (0.1.10).
