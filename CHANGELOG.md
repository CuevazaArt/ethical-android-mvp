...
### Cursor Team Updates
- **Plan 8.1.44 — V14 `local_llm_audit` strips Unicode line/paragraph separators (8.1.41 follow-up):** `scripts/swarm_sync.py` — `_sanitize_one_line_audit_field` also removes U+0085 / U+2028 / U+2029 from `ollama_netloc` and `ollama_model` (not C0, but still break single-line operator audit and log readability). Test: `test_swarm_sync_ollama_audit_strips_unicode_line_separators` in `tests/test_swarm_sync_script.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.44).
- **Plan 8.1.43 — `ENV_VAR_CATALOG` documents `KERNEL_PERCEPTION_PARALLEL*` (8.1.7 / 8.1.39):** `docs/ENV_VAR_CATALOG.md` — new rows for `KERNEL_PERCEPTION_PARALLEL` and `KERNEL_PERCEPTION_PARALLEL_WORKERS` (EXPERIMENTAL flag, 64 worker cap in `kernel_utils.perception_parallel_workers`); `.env.example` comment points to the catalog. **Validation: GitHub Actions** (unchanged `pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.43).
- **Plan 8.1.42 — chat I3 `coercion_uncertainty_raw` rejects `bool` (aligns with 8.1.37):** `src/kernel_lobes/chat_turn_signal_routing.py` — `coercion_report["uncertainty"]` and `coercion_report.uncertainty()` no longer let `float(True)==1.0` inflate `perception_uncertainty` on the chat merge path (Block 0.1.3). Tests: `tests/test_chat_turn_signal_routing.py` (dict + callable). **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.42).
- **Plan 8.1.41 — V14 `local_llm_audit` C0-sanitize for JSONL (swarm hand-off):** `scripts/swarm_sync.py` — `ollama_netloc` and `ollama_model` pass through `_sanitize_one_line_audit_field` (drop control characters & length caps) so poisoned or broken `OLLAMA_*` env values do not smuggle newlines into `data/swarm_sync_log.jsonl` records. Test: `test_swarm_sync_ollama_audit_strips_control_chars_in_model` in `tests/test_swarm_sync_script.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.41).
- **Plan 8.1.40 — `Term.color` defensive `str(text)` (parity with `header` / 8.1.35 follow-up):** `src/utils/terminal_colors.py` — `Term.color` wraps `str` coercion in `(TypeError, ValueError)`; pathological `__str__` falls back to `"?"` in plain and ANSI paths so operator output cannot be taken down by hostile objects. Test: `test_color_str_coercion_failure_falls_back_to_question` in `tests/test_terminal_colors.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.40).
- **Fase 15.22 (Harden-In-Place) — MalAbs anchor TTL is always finite and ≥ 0:** `src/modules/semantic_anchor_store.py` — `_coerce_nonnegative_ttl_s` centralizes validation for `SemanticAnchorRecord.ttl_s`, `SemanticAnchorStore.from_env` (`KERNEL_SEMANTIC_ANCHOR_TTL_S`), `InMemorySemanticAnchorStore` / `ChromaSemanticAnchorStore` `default_ttl_s`, and `create_anchor_store` (`NaN`/`Inf`/invalid strings/negative values → `0.0`, preventing poisoned monotonic and wall-clock expiry). New tests: `test_from_env_nonfinite_or_negative_ttl_coerces_to_zero`, `test_create_anchor_store_coerces_nonfinite_ttl`, `test_record_nonfinite_ttl_coerces_to_zero` in `tests/test_semantic_anchor_store.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: Fase 15.22, MalAbs vector tier.
- **Plan 8.1.39 — `perception_parallel_workers` hard-caps `KERNEL_PERCEPTION_PARALLEL_WORKERS` (Harden-In-Place):** `src/kernel_utils.py` — after `kernel_env_int`, configured worker count is limited to `_MAX_PERCEPTION_PARALLEL_ENV_WORKERS` (64) so optional **EXPERIMENTAL** parallel perception cannot spawn an unbounded local thread pool from a typo or bad env. Test: `test_perception_parallel_workers_env_capped` in `tests/test_kernel_utils.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.39).
- **Plan 8.1.37 — `perception_coercion_u_value` rejects `bool` (backlog §5):** `src/kernel_utils.py` — `type(raw) is bool` returns `None` so `float(True) == 1.0` cannot be misread as full coercion uncertainty; aligns with 8.1.30 / `format_proactive_candidate_line` / 8.1.24. Test: `test_perception_coercion_u_value_rejects_bool` in `tests/test_kernel_utils.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.37).
- **Plan 8.1.38 — ADR 0005 tests: in-memory `NarrativeEpisode` (no Ollama / `register` I/O):** `tests/test_temporal_horizon_prior.py` — `test_registered_episodes_produce_nonzero_signal` uses `_declining_episodes_synthetic()` instead of six `NarrativeMemory.register` calls, avoiding multi-second HTTP embedding timeouts to localhost when Ollama is not running. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.38).
- **Plan 8.1.34 — `Term.header(title: object)` matches `subheader` / `highlight_decision`:** `src/utils/terminal_colors.py` — `str(...).strip()` with safe fallback; `None`/whitespace-only title → `?` in the cyan line (avoids `AttributeError` from `None.strip()`). Test: `test_header_title_coercion_object_like_subheader` in `tests/test_terminal_colors.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.34).
- **Bloque 16.0.11 — `kernel_utils` in Cursor integration gate:** `scripts/eval/run_cursor_integration_gate.py` and `docs/collaboration/CURSOR_CROSS_TEAM_INTEGRATION_GATE.md` list `tests/test_kernel_utils.py` (Fase **15.18** / **0.1.9-0.1.10** / **8.1.30-8.1.31** — env coercers, `format_proactive_candidate_line` / anti-shadow reexports, finite `kernel_env_float`). Complements **16.0.10** Tri-Lobe stack + **8.1.29** `swarm_sync` gate. **Validation: GitHub Actions** (`pytest tests/` + gate).
- **Bloque 0.1.10 / anti-shadow — `src.kernel` reexports vs `kernel_utils`:** `tests/test_kernel_utils.py` — `test_kernel_module_reexports_match_kernel_utils` asserts object identity for the full `kernel_utils` import block in `src/kernel` (ProactivePulse formatter as `_format_proactive_candidate_line`, `kernel_env_*`, `perception_parallel_workers`, and the DAO / mixture facades) so a future duplicate `def` in `kernel.py` cannot shadow imports. PLAN 0.1.10 text lists the same surface. Complements **8.1.30** / **8.1.31**; see gate **16.0.11**. **Validation: GitHub Actions** (`pytest tests/`).
- **Plan 8.1.33 — Temporal horizon prior: finite signals for Bayesian weight nudges (ADR 0005):** `src/modules/temporal_horizon_prior.py` — `compute_horizon_signals` coerces non-finite `np.std` / stability and, before returning, falls back to `(0.0, 0.5, 0.0)` (same as empty history) if `weeks_trend`, `long_term_stability`, or `combined` is not finite, so NaN-heavy episodic scores cannot propagate into `BayesianEngine.hypothesis_weights`. `apply_horizon_prior_to_engine` also gates `sig.combined` before `_nudge_delta` (defense in depth). Tests: `tests/test_temporal_horizon_prior.py` (`test_compute_horizon_signals_all_nan_scores_falls_back`, `test_apply_prior_nonfinite_combined_skips_nudge`). **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.33).
- **Plan 8.1.31 — `kernel_env_float` rejects `NaN` / `Inf` in environment strings:** `src/kernel_utils.py` — `kernel_env_float` now returns the caller default when `float(raw)` is not finite, so e.g. `KERNEL_PROACTIVE_PULSE_IDLE_S=nan` cannot corrupt ProactivePulse idle timing (aligns with 8.1.24’s finite-operator contract). Test: `test_kernel_env_float_nonfinite_falls_back` in `tests/test_kernel_utils.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.31).
- **Plan 8.1.30 — `kernel_utils` narrowing contracts (backlog §5 follow-up):** `tests/test_kernel_utils.py` — `kernel_dao_as_mock` (plain `MockDAO` vs `DAOOrchestrator.local_dao`), `kernel_mixture_scorer` (`BayesianEngine` vs `WeightedEthicsScorer` facade), `format_proactive_candidate_line` rejects `bool` as numeric impact (aligns with 8.1.24 / Tri-Lobe ProactivePulse). **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.30).
- **Plan 8.1.29 — Cursor integration gate includes V14 `swarm_sync` contract:** `scripts/eval/run_cursor_integration_gate.py` now runs `tests/test_swarm_sync_script.py` (subprocess `--dry-run`, `local_llm_audit` / `OLLAMA_BASE_URL` netloc) so the operator hand-off JSON for `scripts/swarm_sync.py` cannot drift without failing the pre-merge gate. `docs/collaboration/CURSOR_CROSS_TEAM_INTEGRATION_GATE.md` updated. **Validation: GitHub Actions** (full `pytest tests/`; gate reuses the same file). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.29).
- **Plan 8.1.28 — `Term.subheader` coerces `title: object` (operator parity with 8.1.20):** `src/utils/terminal_colors.py` — `str(...).strip()` with `None`/whitespace-only → dim `?`, matching `highlight_decision`. Test: `test_subheader_coercion_matches_decision_contract` in `tests/test_terminal_colors.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.28).
- **Plan 8.1.27 — `_clamped_header_bar_width` saturates *default* (defense in depth):** `src/utils/terminal_colors.py` — new `_saturate_bar_width`; all branches of `_clamped_header_bar_width`, including fallbacks to *default* (`bool`/`None`/parse failures), return a width in `[1, _MAX_HEADER_BAR]`, so an oversized *default* cannot escape the public helper (complements 8.1.26). Test: `test_clamped_header_bar_width_saturates_default_on_invalid_raw`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.27).
- **Plan 8.1.26 — `Term.header` invalid explicit `width` falls back to `_rule_width()` (Harden-In-Place):** `src/utils/terminal_colors.py` — for non-`None` *width*, `_clamped_header_bar_width` now uses `default=cls._rule_width()` (capped) instead of raw `cls.SEP_WIDTH`, so values that return the default verbatim (e.g. `bool` *width* on a wide-`SEP_WIDTH` subclass) cannot exceed `_MAX_HEADER_BAR`. Test: `test_header_invalid_width_fallback_uses_clamped_default`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.26).
- **Fase 15.20 (Boy Scout / Harden-In-Place) — Chroma anchor TTL + `delete` observability:** `src/modules/semantic_anchor_store.py` — `ChromaSemanticAnchorStore.delete` now logs **DEBUG** with `exc_info` (matches other Chroma methods; closes the silent `except` gap). `delete_expired` validates `before_timestamp` and per-row metadata `timestamp` with safe `float` and `math.isfinite`, and wraps the Chroma get/delete path in a single `try` with **DEBUG** on failure. Test: `test_chroma_delete_failure_emits_debug` in `tests/test_semantic_anchor_store.py` (Chroma; skipped if `chromadb` absent). **Validation: GitHub Actions** (`pytest tests/`). Plan: Fase 15.20, MalAbs vector tier.
- **Fase 15.19 (Boy Scout) — Chroma `SemanticAnchorStore` failures are debug-visible:** `src/modules/semantic_anchor_store.py` — `ChromaSemanticAnchorStore` logs **DEBUG** with `exc_info` on `query_neighbors` / `delete` / `get` / `get_all_anchors` failures (best-effort returns unchanged). Distance→similarity uses safe `float` and `math.isfinite` on distance and similarity. Test: `test_chroma_query_neighbors_failure_emits_debug` in `tests/test_semantic_anchor_store.py` (when `chromadb` is installed; otherwise skipped). **Validation: GitHub Actions** (`pytest tests/`). Plan: Fase 15.19, MalAbs vector tier.
- **Fase 15.18 (MINOR_CONTRIBUTIONS_BACKLOG §5) — `kernel_utils` env + perception helpers, slimmer `kernel`:** `src/kernel_utils.py` now holds `kernel_env_truthy` / `kernel_env_int` / `kernel_env_float`, `perception_parallel_workers` (read-only `os.environ` + `os.cpu_count`), and `perception_coercion_u_value` (clamped, `math.isfinite`-safe) alongside `format_proactive_candidate_line`. `src/kernel.py` imports them as the existing `_*` aliases, drops duplicated function bodies, and removes a mistaken duplicate `checkpoint`/`deprecation` import block. New contract tests: `tests/test_kernel_utils.py`. `src/MANIFEST.json` tracks digests for `src/kernel.py` and `src/kernel_utils.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: **Fase 15.18** + **Bloque 0.1, Tarea 0.1.9** (anti–God-object hygiene, single narrative).
- **Fase 15.17 (Boy Scout / ADR 0003) — `semantic_chat_gate` MalAbs path is observable, cancels propagate:** `src/modules/semantic_chat_gate.py` — `llm_backend.embedding` failures are logged at **DEBUG** and still drive `observe_embedding_error` (narrow `except` tuple) instead of blind swallow; `add_semantic_anchor` and best-effort `query_neighbors` / TTL no longer use silent `pass` (warnings with `exc_info` where the operator needs visibility, **DEBUG** for hot-path fallbacks to legacy cache); `complete` / `acomplete` arbiter paths re-raise `LLMHttpCancelledError` (cooperative cancel must not be converted into a fail-closed `AbsoluteEvilResult`). Built-in anchor import-time init remains *after* `_fetch_embedding_with_fallback` (per Plan 8.1.24). New tests: `test_add_semantic_anchor_store_failure_is_logged`, `test_backend_embedding_error_emits_debug_log`. **Validation: GitHub Actions** (`pytest tests/`). Plan: Fase 15.17, semantic MalAbs.
- **Plan 8.1.25 — `Term.header` default width follows `_rule_width()` (subclass + rules parity):** `src/utils/terminal_colors.py` — `header` defaults `width=None` and uses `_rule_width()` (same as `rule_heavy` / `rule_light` without an override) instead of a fixed `70`-class literal, so a subclass `SEP_WIDTH` is reflected in the `═` bar. Test: `test_header_default_width_follows_subclass_sep_width` in `tests/test_terminal_colors.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.25).
- **Plan 8.1.24 — Tri-Lobe ProactivePulse (idle) finite operator line:** `src/kernel.py` — `_format_proactive_candidate_line` coerces `estimated_impact` / `confidence` (finite, not `bool`), clamps ranges, caps one-line description; `CorpusCallosumOrchestrator._async_proactive_pulse` uses it. `src/modules/semantic_chat_gate.py` — single `_populate_builtin_anchors_to_store` + import-time call placed **after** `_fetch_embedding_with_fallback` (fixes `NameError` and duplicate definition). Tests: `tests/test_kernel_lobes_stack.py` (`test_format_proactive_candidate_line_clamps_nonfinite_impact_conf`, `test_corpus_callosum_proactive_idle_uses_finite_operator_line`). **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.24).
- **Plan 8.1.22 / operator DX — `Term.rule_heavy`/`rule_light` `width=` + `_DEFAULT_SEP_WIDTH` (closes 8.1.19 runtime gap):** `src/utils/terminal_colors.py` — optional `width=` (same clamp as `Term.header`); module `_DEFAULT_SEP_WIDTH` removes scattered `70` in `Term.SEP_WIDTH`, `_rule_width` fallbacks, and the `header` default. Fixes `TypeError: unexpected keyword argument 'width'` in `test_rule_heavy_and_light_respect_clamped_width` so CI matches PLAN 8.1.19. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.22).
- **Plan 8.1.21 / Fase 15.7 — `atomic_io` + plan truth (pathlib `TypeError`):** `src/persistence/atomic_io.py` — `__all__` (`atomic_write_bytes`); regresión `TypeError` por `data` no-`bytes` sigue limpiando `*.tmp.*` vía el mismo manejador. `PLAN` **Tarea 15.7** corregida: excepciones explícitas `(OSError, TypeError)` (antes decía `ValueError`, desalineado a pathlib/8.1.8). Test: `test_atomic_write_bytes_typeerror_for_non_bytes_no_orphan_tmp`. **Validation: GitHub Actions** (`.github/workflows/ci.yml`).
- **Bloque 16.0.8 — integration gate: salience + somatic (anti-orphan):** `scripts/eval/run_cursor_integration_gate.py` now includes `tests/test_salience_map.py` and `tests/test_somatic_markers.py` (aligns with PLAN 8.1.4 / 8.1.6 finite-path contracts). `docs/collaboration/CURSOR_CROSS_TEAM_INTEGRATION_GATE.md` updated. **Full repo validation: GitHub Actions** on `pytest tests/`; gate script is an operator-local subset.
- **Plan 8.1.19 — `Term.rule_heavy` / `rule_light` share header width policy:** `src/utils/terminal_colors.py` — `Term._rule_width()` clamps `SEP_WIDTH`; optional per-call `width=` uses `_clamped_header_bar_width` (512 cap, same as `Term.header`); prevents huge `=`/`-` runs for oversized `SEP_WIDTH` or operator overrides. Tests: `test_rule_heavy_and_light_respect_clamped_width`, `test_rule_bars_clamp_when_subclass_sep_width_huge` in `tests/test_terminal_colors.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.19).
- **Plan 8.1.5 — Tri-lobe limbic: finite swarm trust nudge:** `LimbicEthicalLobe.execute_stage` applies `safe_signal_scalar` to `get_swarm_trust_nudge()` so `NaN`/`Inf` cannot saturate `signals["trust"]`; narrow `except` tuple (replaces bare `Exception`). **Validation: GitHub Actions** (`tests/test_kernel_lobes_stack.py::test_limbic_execute_stage_ignores_nonfinite_swarm_nudge`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.5).
- **Plan 8.1.18 — `terminal_colors` public width helper (operator / demos):** `src/utils/terminal_colors.py` — `_clamped_header_bar_width` is included in `__all__` alongside `Term` and `_MAX_HEADER_BAR` so operator scripts and gates reuse the same finitude / cap as `Term.header` without forking the logic. Test: `test_clamped_header_bar_width_is_public_and_matches_header_logic` in `tests/test_terminal_colors.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.18).
- **Plan 8.1.14 / operator DX — `Term.header` width: numeric `str`:** `src/utils/terminal_colors.py` — `_clamped_header_bar_width` accepts `str` (e.g. env-sourced) via `float`→`int` with the same finitude cap; invalid text uses `default=Term.SEP_WIDTH`. Tests: `test_header_bar_width_accepts_numeric_str`, `test_header_bar_width_bad_str_falls_back_to_default` in `tests/test_terminal_colors.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.14).
- **Plan 8.1.15 — `Term.header` int width: no post-coercion `float(w)` (Harden-In-Place):** `src/utils/terminal_colors.py` — the unified clamp must not end with `float` on a plain `int` width, or `float(10**400)` is `inf` and the `═` bar vanishes. Dedicated `int` and finite `str` paths; `None` uses default; `highlight_impact` also catches `OverflowError` on `float(...)`. Test: `test_header_bar_width_huge_int_does_not_use_float_of_int`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.15).
- **Plan 8.1.13 / operator DX — `Term.header` bar width (anti-DoS):** `src/utils/terminal_colors.py` — single `_clamped_header_bar_width` (rejects `bool` and non-finite widths, caps at `_MAX_HEADER_BAR = 512`, `default=Term.SEP_WIDTH`); removed duplicate width helper; `__all__` documents `Term` + `_MAX_HEADER_BAR` for tests/gates. `tests/test_terminal_colors.py` (`test_header_bar_width_clamped_and_safe` includes `NaN` width). **Validation: GitHub Actions** (`pytest tests/`).
- **Bloque 16.0.9 — Tri-Lobe bus memory in Cursor integration gate:** `scripts/eval/run_cursor_integration_gate.py` and `docs/collaboration/CURSOR_CROSS_TEAM_INTEGRATION_GATE.md` list `tests/test_kernel_tri_lobe_bus_memory.py` (PLAN **26.0.1**, MemoryLobe ↔ `EVENT_KERNEL_*` amnesia / episode registration; anti-drift with Tri-Lobe, complements **16.0.8** salience/somatic). **Validation: GitHub Actions** (`pytest tests/` + gate).
- **Bloque 16.0.10 — Tri-Lobe stack tests in Cursor integration gate:** `scripts/eval/run_cursor_integration_gate.py` and `docs/collaboration/CURSOR_CROSS_TEAM_INTEGRATION_GATE.md` list `tests/test_kernel_lobes_stack.py` (PLAN **8.1.5** / **8.1.24** — finite swarm nudge, ProactivePulse operator line, `CorpusCallosum` paths; gate parity with code already cited in `CHANGELOG` for 8.1.5/8.1.24). **Validation: GitHub Actions** (`pytest tests/` + gate).
- **Bloque 16.0.7 — PLAN 16.0.4/8.1.x doc truth (V4.0):** `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` — tarea **16.0.4** ahora cita `_MAX_HEADER_BAR` = 512 y **8.1.13** (sustituye tope 200 desactualizado); **8.1.17** separa el cierre *OverflowError* de **8.1.16** (sin duplicado de numeración). Sin cambio de runtime. **Validation: GitHub Actions** inalterada.
- **Bloque 16.0.6 — PerceptiveLobe in Cursor integration gate:** `scripts/eval/run_cursor_integration_gate.py` and `docs/collaboration/CURSOR_CROSS_TEAM_INTEGRATION_GATE.md` list `tests/test_perceptive_lobe.py` (PLAN 16.0.5, Tri-Lobe MOCK / `aclose` anti-drift). **Validation: GitHub Actions** (`pytest tests/` + gate).
- **Fase 15.13 (Boy Scout) — `/health` version string is not a catch-all:** `src/chat_server.py` — `_package_version` uses `importlib.metadata.PackageNotFoundError` instead of `except Exception` when `ethos-kernel` is not installed (returns `"dev"` as before). Test: `test_package_version_falls_back_to_dev_when_not_installed`. **Validation: GitHub Actions** (`tests/test_chat_server.py` + full suite). Plan: Fase 15.13, Bloque 0.2 (operador HTTP).
- **Fase 15.10 (Boy Scout) — `llm_backends` HTTP JSON + Ollama embed:** `src/modules/llm_backends.py` — `_http_response_json_dict` uses concrete parse/body errors (no bare `Exception` on `response.json()` / `response.text`); `OllamaLLMBackend.embedding` returns `None` on `httpx.HTTPError` or `ValueError` (after `raise_for_status` / `_http_response_json_dict`) with `logging.debug` instead of `except Exception: return None`. **Validation: GitHub Actions** (`tests/test_llm_backend_adapters.py` + full suite). Plan: Fase 15.10, Bloque 0.1.1.
- **Fase 15.14 (Boy Scout) — WebSocket proactive cancel + draft errors + terminal import fix:** `src/chat_server.py` — proactive idle pulse re-raises `asyncio.CancelledError`; `constitution_draft` path catches only `(TypeError, ValueError)` (aligned with `add_constitution_draft` / `int(level)`), not all `Exception`. `src/utils/terminal_colors.py` — `Term.SEP_WIDTH` set to `70` (fixes `NameError` from undefined `_DEFAULT_BLOCK_WIDTH` that broke `import src.chat_server`). **Validation: GitHub Actions** (`pytest tests/`). Plan: Fase 15.14, Bloque 0.2.1.
- **Fase 15.15 (Boy Scout) — `internal_error` send path on `chat_turn_error`:** `src/chat_server.py` — best-effort `ws.send_json` after a turn failure is no longer `except Exception`; uses `(WebSocketDisconnect, OSError, RuntimeError, TypeError)` so unexpected send failures are not swallowed as debug-only. **Validation: GitHub Actions** (`pytest tests/`). Plan: Fase 15.15, Bloque 0.2.1.
- **Bloque 8.1.10 — Charm `frustration_streak` regression (V4.0):** `tests/test_charm_engine.py` — `test_as_nonneg_int_streak_sanitizes_caution_correlates` locks non-finite/invalid streak inputs so `StyleParametrizer` play caps do not follow poisoned `UserModel` streaks; `charm_engine` module doc English + explicit **EXPERIMENTAL / MOCK** for the style/gesture stack. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (8.1.10).
- **Bloque 0.1.8 (L1) — Charm Engine: finite `caution` / intimacy / streak before `gesture_plan`:** `src/modules/charm_engine.py` — `_as_unit01` and `_as_nonneg_int_streak` so poisoned trust scalars cannot produce **NaN**/**Inf** in `CharmVector` or `GesturePlanner` metadata (MOCK/EXPERIMENTAL path; Nomad bridge–aligned). `tests/test_charm_engine.py`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (0.1.8).
- **Plan 8.1.8 — `atomic_write_bytes` (checkpoint durability):** `src/persistence/atomic_io.py` — failures during `Path.write_bytes` or `os.replace` are catchable as `(OSError, TypeError)`; partial temp `*.tmp.<pid>` is removed with `_try_unlink_tmp` (best-effort `unlink`, race-safe) before the original error propagates, so spurious work is not misclassified. Tests: `test_atomic_write_bytes_persists_content`, `test_atomic_write_bytes_cleans_tmp_on_replace_failure` in `tests/test_persistence.py`. **Validation: GitHub Actions** (`.github/workflows/ci.yml`).
- **Plan 8.1.6 — Somatic markers finite nudges (v10):** `src/modules/somatic_markers.py` — `_finite_signal01` / `_clamp01` coercen entradas no finitas; `learn_negative_pattern` no persiste si el peso no es finito; `apply_somatic_nudges` sanea `risk` / `urgency` / `calm` antes de sumar el término somático, evitando **NaN** en el diccionario de señales. Regresión: `tests/test_v10_operational.py` (`test_somatic_learn_ignores_nonfinite_weight`, `test_somatic_nudges_sanitize_nonfinite_signal_bases`). **Validación: GitHub Actions** (`pytest tests/`).
- **Bloque 0.1.7 / ADR 0002 — `run_burst_cancel_smoke` CLI contract (argparse exit 2):** `tests/test_run_burst_cancel_smoke_script.py` — subprocess checks that invalid `--delay` / `--join-timeout` / `--workers` / `--cancel-after` fail before the mock thread pool (complements Bloque 0.1.5 parse-time guards). **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (0.1.7).
- **Bloque 0.1.6 / Pragmatismo V4.0 — `merge_sensor_hints` anti-poison:** `src/modules/sensor_contracts.py` — `_clamp01` maps non-finite inputs to `0.5`; after copying `signals`, the five canonical keys are clamped so paths that do not nudge a key cannot leave `NaN`/`Inf` in the outgoing map; `stability_score` low-stability nudge uses an explicit finite-`float` guard (excludes `bool`). Test: `tests/test_sensor_contracts.py::test_merge_clamps_nonfinite_incoming_signal_values`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (0.1.6).
- **Bloque 16.0.4 — `Term.header` width hardening + gate:** `src/utils/terminal_colors.py` — `__all__`, `_clamped_header_bar_width` (capped at `_MAX_HEADER_BAR`, rejects bool/invalid/non-finite inputs); `tests/test_terminal_colors.py`. `run_cursor_integration_gate.py` includes `test_terminal_colors.py`. **Validation: GitHub Actions** (`pytest tests/`).
- **Bloque 16.0.3 — SomaticMarker exports + integration gate wire:** `src/modules/somatic_markers.py` — `__all__`; `_clamp01(x: object)`; `scripts/eval/run_cursor_integration_gate.py` includes `tests/test_variability_engine.py` (Plan 16.0.2 anti-drift). **Validation: GitHub Actions** (`pytest tests/`).
- **Bloque 16.0.2 — VariabilityEngine hardening (Bayesian noise):** `src/modules/variability.py` — `_finite_or` / `_finite_out` for non-finite inputs and post-`clip` results; non-finite Gaussian draw returns the coerced base; `__all__`; `salience_map.py` exports `__all__`. Tests: `tests/test_variability_engine.py`. **Validation: GitHub Actions** (`pytest tests/`).
- **Bloque 16.0.1 — vision pilot script (MOCK/EXPERIMENTAL, lab only):** `scripts/run_vision_pilot_validation.py` — removed orphan `_run_perception_stage` stub; `MockLLM` implements `acommunicate` and `anarrate` so `process_natural`’s async path is honest; `math.isfinite` check on moral score. Not a CI contract. **Validation: optional** `python scripts/run_vision_pilot_validation.py`; production truth remains **GitHub Actions** on `tests/`.
- **Fase 15.6 (Boy Scout) — JSON checkpoint Fernet fallback observable:** `src/persistence/json_store.py` — `JsonFilePersistence.load` uses `InvalidToken` / `UnicodeDecodeError` / `ValueError` (not bare `Exception`) and logs **INFO** before treating the file as plain UTF-8 JSON (legacy migration with `KERNEL_CHECKPOINT_FERNET_KEY` set). `Fernet` import at module level. Test: `test_json_encrypted_load_fallback_plain_file_emits_info_log`. **Validation: GitHub Actions** (`.github/workflows/ci.yml`).
- **Fase 15.5 (Boy Scout) — evidence key observability + cancel validation:** `src/modules/evidence_safe.py` — invalid `fernet_key` / `KERNEL_EVIDENCE_KEY`: ``except (TypeError, ValueError)`` and **WARNING** (replaces bare ``Exception``) before transient Fernet; hashing still works. `llm_cancel_burst` already uses ``numbers.Real``/finite checks; new tests: `test_evidence_safe_invalid_fernet_key_falls_back_with_warning`, `test_run_burst_cancel_smoke_rejects_nonfinite_completion_delay`. **Validation: GitHub Actions** (`.github/workflows/ci.yml`).
- **Plan 0.1.5 / ADR 0002 — CLI smoke alineada al núcleo (finito en parseo):** `scripts/eval/run_burst_cancel_smoke.py` — `argparse` con `type=` (``math.isfinite``; ``--cancel-after`` ≥ 0; ``--workers`` en ``[1, MAX_BURST_SMOKE_WORKERS]``) para fallar **antes** de reservar el pool; doc **EXPERIMENTAL / MOCK**. `llm_cancel_burst` exporta `MAX_BURST_SMOKE_WORKERS` (alias de ``_MAX_BURST_SMOKE_WORKERS``) para un solo tope operador↔biblioteca. **Validación: CI** (Bloque 0.1; ``pytest`` sigue en ``test_llm_cancel_burst_operational``).
- **Plan 0.1.4 / ADR 0002 — burst-cancel smoke, parámetros finitos (V4.0 cierre):** `src/modules/llm_cancel_burst.py` — ``_MAX_BURST_SMOKE_WORKERS = 4096`` (humo, no carga); ``run_burst_cancel_smoke`` docstring marca trayectoria **MOCK**; ``_validate_run_burst_params`` (``math.isfinite``; ``n_workers`` estrictamente ``int``; retrasos finitos y signo; ``cancel_after_s`` ≥ 0) evita ``NaN``/``Inf`` en ``time.sleep``. Tests: ``tests/test_llm_cancel_burst_operational.py`` (p. ej. ``n_workers=10_000``, ``cancel_after_s=nan``). **Validación: CI** (Bloque 0.1).
- **Plan 8.1.4 — SalienceMap finite inputs + uniform sum fix:** `src/modules/salience_map.py` — `_fin_unit_interval`; uniform fallback uses `1/len(AXIS_ORDER)` (was 0.25×5 axes → **1.25**, invalid). **Hotfix:** `EthicalKernel.aprocess` — `KernelDecision` was wrongly nested under `else` of `register_episode`, causing `UnboundLocalError` when `register_episode` is true (regression caught by `test_kernel_has_salience`). **Validation: GitHub Actions** (`pytest tests/`). *(Backlog: `MINOR_CONTRIBUTIONS_BACKLOG.md` salience item.)*
- **Fase 15 (L0) cierre de plano:** `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` — cabecera de **Fase 15** pasa a **[DONE]** (antes *CORE DONE*), alineada con 15.0.1–15.4. Sin cambio de runtime. Cierre: `swarm_sync` (bloque 15.0) + este changelog.
- **Fase 15.4 — Motivation report + seek_internal (finiteness):** ``_sanitize_motivation_report_for_bus`` for the ``motive`` field on ``EVENT_KERNEL_PROACTIVE_PULSE``; ``seek_internal_purpose`` clamps ``estimated_impact`` / ``confidence`` for both ``CandidateAction`` and ``dict`` raw rows. **Validation: CI**; local ``py_compile src/kernel.py`` only.
- **Fase 15.3 — ProactivePulse bus payload (sustracción de NaN):** ``EthicalKernel.emit_proactive_pulse_if_idle`` (``src/kernel.py``) clamps/sanitizes ``estimated_impact`` / ``confidence`` and caps ``idle_seconds`` before ``EVENT_KERNEL_PROACTIVE_PULSE``; avoids non-finite telemetry on the in-process bus. **Validation: CI** (per L0 Fase 15; local ``pytest`` collection not assumed).
- **Fase 15.0.3 — PLAN S.2.1 doc merge (no code):** ``docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md`` — una sola línea **[DONE]** para tarea S.2.1 (sustituye doble marca de cierre y resume entregable en un párrafo). Cierre: ``python scripts/swarm_sync.py`` (Fase 15).
- **Fase 15.2 — Nomad jerk passthrough (consolidation):** ``_coerce_nomad_telemetry_for_snapshot`` no longer normalizes ``jerk``/``shock`` into ``[0,1]``; values stay finite and match ``assess_vitality`` / ``_finite_jerk`` (Fase 15, PLAN). Test ``tests/test_vitality.py::test_merge_nomad_passthrough_raw_jerk_matches_assess``. **Validation: CI.**
- **Plan S.2.1 — Termo-Visual (Nomad → vitality):** Advisory thermal band: ``KERNEL_VITALITY_THERMAL_WARN_C`` (default 70°C), ``thermal_elevated`` on :class:`VitalityAssessment`, hints + ``build_perception_confidence_envelope(..., thermal_elevated=...)`` (light penalty) and ``merge_sensor_hints_into_signals`` nudges; Nomad ``telemetry`` aliases (``device_temp_c``, ``cpu_temp_c``, etc.) coerced in ``_coerce_nomad_telemetry_for_snapshot`` before :class:`SensorSnapshot` merge. Documented in ``docs/ENV_VAR_CATALOG.md``. **Validation: CI** (``.github/workflows/ci.yml``).
- **Fase 15 / Plan S.2.1 — Vitality + Nomad (finite telemetry):** ``src/modules/vitality.py`` — ``_finite_jerk`` (accelerometer magnitude may exceed 1.0; not clamped to ``[0,1]`` like battery); ``KERNEL_VITALITY_CRITICAL_TEMP`` rejects non-finite and non-positive values (fallback 80.0). ``vitality_communication_hint`` maps non-finite ``trust_level`` to the untrusted disclosure path. Tests: ``tests/test_vitality.py`` (e.g. ``test_vitality_hint_nonfinite_trust_is_untrusted``). **Validation: CI** (``.github/workflows/ci.yml``); local `pytest` collection can be noisy.
- **Fase 15 (L0) — sustracción / consolidación:** Removed `INFO` ``DEBUG:`` lines from ``process_chat_turn_stream`` (``src/kernel.py``), including the residual ``DEBUG: after malabs`` (Fase 15.0.2); ``src/MANIFEST.json`` digest for ``src/kernel.py`` updated. Replaced bare ``except: pass`` in ``src/chat_server.py`` WebSocket handlers with ``except Exception`` + ``logger.debug`` (failure to send ``internal_error``; constitution draft handler). **Validation: CI** (``.github/workflows/ci.yml``); local `pytest` collection may be noisy.
- **Plan C.1.2 — RLHF drag on Stage-3 mixture:** Regression tests ``tests/test_rlhf_c12_stage3_mixture_drag.py`` — with ``KERNEL_RLHF_MODULATE_BAYESIAN``, ``EthicsMixtureResult.applied_mixture_weights`` changes after ``maybe_modulate_from_malabs_rlhf_features``; no-op when the flag is unset. See ``docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md`` (C.1.2).
- **Plan C.1.1 (chat path wiring):** After MalAbs in ``process_chat_turn_stream`` and ``process_chat_turn_async``, ``mal.rlhf_features`` is copied onto merged ``signals`` so ``EthicalKernel.aprocess`` can ``await apply_rlhf_modulation_to_bayesian_async`` before Bayesian scoring (same contract as ``merge_chat_turn_signals_for_ethical_core`` shallow-copy).
- **Plan C.1.1 — RLHF reward → Bayesian priors:** ``src/modules/rlhf_reward_model.py`` — ``feature_vector_from_malabs_dict`` / ``reward_predict_from_malabs_dict``, ``apply_rlhf_modulation_to_bayesian*`` (artifact cache). ``feature_vector_from_malabs_dict`` returns ``None`` for missing/empty payloads (bridge no-op); ``import asyncio`` for ``RewardModel.apredict``. ``src/modules/bayesian_engine.py`` — optional ``KERNEL_RLHF_MODULATE_USE_TRAINED_MODEL``; ``maybe_modulate_from_malabs_rlhf_features``; ``apply_rlhf_modulation`` sanitizes non-finite inputs. Tests: ``tests/test_bayesian_rlhf_c11.py``, ``tests/test_rlhf_bayesian_bridge.py``. Tri-lobe ``aprocess`` awaits async bridge before Bayesian stage; chat stream/async copy ``mal.rlhf_features`` into ``signals`` (see bullet above).
- **Plan 8.1.2 — terminal DX / CI gate:** `src/utils/terminal_colors.py` — `Term.SEP_WIDTH`, `rule_heavy` / `rule_light`; **`NO_COLOR`** (presence) and **`KERNEL_TERM_COLOR`** via `_colors_enabled()`. **`tests/test_terminal_colors.py`**. WebSocket: `test_websocket_max_message_bytes_zero_disables_cap` uses `_ws_pop_chat_turn_payload` (streaming `turn_started` before final JSON). Episodic weights: `test_kernel_episodic_flag_changes_weights` sets **`KERNEL_NARRATIVE_DB_PATH=:memory:`**. GitHub Actions **`.github/workflows/ci.yml`** — job **`ci-all-passed`** aggregates **`compose-validate`**, **`quality`** (ruff, format, mypy, full `pytest tests/` on 3.11/3.12), **`semantic-default-contract`**, **`windows-smoke`** (full `pytest tests/` on Windows).
- **Bloque 0.1 (kernel LLM HTTP — Boy Scout):** ``_http_response_json_dict`` in ``src/modules/llm_backends.py`` for Ollama + HTTP JSON bodies (fail-fast ``ValueError`` on bad JSON / non-object); cooperative ``raise_if_llm_cancel_requested`` on Ollama embedding entry and in ``acompletion_stream`` line loop; ``LLMHttpCancelledError`` is not swallowed by ``embedding()``. Tests: ``tests/test_llm_backend_adapters.py``. **Plan:** ``docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md`` — tareas **0.1.1**, **0.1.2**, **S.1.1** (Nomad), **8.1.1** (tipos / ``chat_turn_policy``) alineadas a **DONE** con el código entregado.
- **CI / Plan 8.1.3 + Windows parity:** GitHub Actions triggers on ``main`` and ``master-*`` (integration hubs); workflow file ``.github/workflows/ci.yml``. The **Windows** job runs full ``pytest tests/`` with the same ``--cov-fail-under`` as Ubuntu, plus ``ruff check`` and ``ruff format --check``. Homoglyph matrix extended in ``tests/fixtures/input_trust_homoglyphs.py`` (Cyrillic IE/о; Greek omicron in *how*). See ``.github/workflows/ci.yml``.
- **MalAbs / chat core:** `VerbalResponse` in `process_chat_turn_stream` and `_process_chat_turn_async_impl` now passes `hax_mode` + `inner_voice` on kernel-block paths (matches `LLMModule` dataclass). **`tests/fixtures/input_trust_homoglyphs.py`** + parametrized lexical regression in **`tests/test_input_trust.py`** (confusable-fold on/off, benign `how 2 …` wording). `verify_collaboration_invariants.py` clean.
- **V14 `scripts/swarm_sync.py`:** CLI `--msg`, `--block`, `--author`, `--ticket`, `--log` (JSONL path, default `data/swarm_sync_log.jsonl`), `--dry-run`; stdout line includes `llm_resolved_mode`, `KERNEL_LLM_CLOUD_DISABLED`, **`local_llm_audit`** (`ollama_netloc`, `ollama_model` from env — matches `OLLAMA_BASE_URL` / `OLLAMA_MODEL` defaults in `llm_layer`, no secrets), optional `somatic_biographic_anchor` from disk, nested `v14` from `v14_session_sync_record` (biography `canonical_summary` prefix from `KERNEL_CEREBELLUM_BIOGRAPHY_PATH`); without `--dry-run`, appends via `append_swarm_sync_event` and JSONL. Tests: `tests/test_swarm_sync_script.py`.
- **Blocks 20.0 / 21.0 — cerebellum biography matrix:** `src/modules/cerebellum_biography.py` persists narrative identity overlay (swarm tail, coherence EMA); `src/modules/session_sync_digest.py` provides V14 fingerprints; `EthicalKernel` merges somatic overlay into the limbic stage and augments Bayesian `signals` when `KERNEL_CEREBELLUM_BIOGRAPHY=1`, with `after_decision` on episode registration. Tests: `tests/test_cerebellum_biography.py`, `tests/test_session_sync_digest.py`.
- **Blocks 20.0 / 21.0 — local LLM fluency:** `LLMModule` now resolves mode with `resolve_llm_mode`, so `KERNEL_LLM_CLOUD_DISABLED=1` prevents selecting the Anthropic client even when `LLM_MODE=api` and a key is present. Documented `KERNEL_LLM_CLOUD_DISABLED` and cerebellum biography env hints in `.env.example`; tests in `tests/test_llm_phase3.py`.
...
- **`scripts/seal_manifest.py`** now prepends the same RoT anchor bytes as `SecureBoot.compute_file_hash` so manifest digests match runtime verification (fixes spurious secure-boot failures after `main` merge).
...
- **Module 9.1 (Rojo / streaming vision):** `NomadBridge.vision_queue_threadsafe` mirrors accepted LAN JPEG frames for the `VisionContinuousDaemon` (asyncio queues are not safe off-loop); `VisionInferenceEngine.analyze_jpeg_bytes` + worker-thread inference; optional MobileNet path via `KERNEL_VISION_DAEMON_CNN`; `KERNEL_VISION_CONTINUOUS_DAEMON` gates the daemon thread in `PerceptiveLobe`; `NomadVisionConsumer` drains `queue.Queue` with `asyncio.to_thread` (fixes invalid `await` on blocking `get`). Queue stats schema `nomad_bridge_queue_stats_v4` with `vision_sync_queued`. See `tests/test_vision_continuous_daemon.py`.
- **Module S.2.1 (Rojo / Nomad → vitality):** `apply_nomad_telemetry_if_enabled` in `PerceptiveLobe._chat_assess_sensor_stack` merges `peek_latest_telemetry()` before `assess_vitality` / multimodal (`KERNEL_NOMAD_TELEMETRY_VITALITY`, default on). `VitalityAssessment.thermal_elevated` (warning band), optional thermal hysteresis latch (`KERNEL_VITALITY_THERMAL_HYSTERESIS`, `KERNEL_VITALITY_THERMAL_HYSTERESIS_C`, `KERNEL_VITALITY_WARNING_TEMP`); `KERNEL_VITALITY_IMPACT_JERK_THRESHOLD` for impact; extended Nomad telemetry key aliases for `core_temperature`; `vitality_communication_hint` includes elevated-thermal advisory. `docs/ENV_VAR_CATALOG.md` lists vitality envs. See `tests/test_vitality.py`.
...
- [x] **Tribunal ├ëtico Edge (Fase 11.4 COMPLETADA)**:
    - **Nivel 1 (Edge Gate)**: Inyecci├│n de `AbsoluteEvilDetector.evaluate_chat_text_fast` en el l├│bulo de percepci├│n para bloqueos < 10ms.
    - **Nivel 2 (As├¡ncrono)**: Refactorizaci├│n del L├│bulo L├¡mbico y Social a ejecuci├│n as├¡ncrona para estabilizar el TTFT.
    - **Consolidaci├│n del Hub**: Sincronizaci├│n final de `master-antigravity` con el suite completo de validaci├│n (85/86 tests corregidos).
    - **Infraestructura**: Implementaci├│n de prefetching de Bridge Phrase (MER V2) y endpoint `/nomad/migration`.
- [x] Inyecci├│n de Tribunal ├ëtico Edge (Nivel 1: Lexicogr├ífico < 50ms) en `PerceptiveLobe`.
- [x] Instanciaci├│n de L├│bulo L├¡mbico como Nivel 2 (As├¡ncrono) para regular TTFT.
- [x] Refactorizaci├│n de `EthicalKernel` para eliminar redundancias en el gate de seguridad.
- [x] Resoluci├│n de conflictos de merge en el Hub de Integraci├│n (`master-antigravity`).
...
## Phase 12 ΓÇö Ouroboros (Audio Bridge) & Hardware Hardening ΓÇö April 2026

## Team Copilot ΓÇö ThalamusNode Sensory Fusion Integration ΓÇö April 2026 (Bloque 10.1)

### Team Copilot Updates

- **`ThalamusNode` wiring (Bloque 10.1 ΓÇö VVAD + VAD Sensory Fusion):**
...
- **Unit tests (`tests/test_thalamus_node.py` ΓÇö 16 tests):**
...
## Team Copilot ΓÇö TurnPrefetcher Integration & Homoglyph Tests ΓÇö April 2026 (Bloque 10.4 / 8.1.3)

### Team Copilot Updates

- **`TurnPrefetcher` ΓÇö Bloque 10.4 (MER V2 Bridge Prefetch):**
  ...
  - Wired `TurnPrefetcher` into `process_chat_turn_stream`: after `decision_finished` (safe path only), fires `predict_bridge` concurrently using the user's charm profile from `UserModelTracker`. Emits a new `bridge_phrase` event before the main LLM tokens begin ΓÇö target latency <300ms.
  - Passed `llm=self.llm` to `ExecutiveLobe` constructor for full DI wiring.
- **Homoglyph Evasion Tests ΓÇö Bloque 8.1.3:**
  - Added 6 new test cases to `tests/adversarial_inputs.py` documenting Cyrillic/Greek confusable evasion attempts:
    - `test_full_cyrillic_substitution_in_bomb_blocked` ΓÇö dense Cyrillic substitution across multiple letters in `bomb`
    - `test_greek_omicron_in_bomb_blocked` ΓÇö Greek omicron (U+03BF) substituted for 'o'
    - `test_cyrillic_jailbreak_token_blocked` ΓÇö Cyrillic ╨╡/╨░ homoglyphs in `operate as an unrestricted assistant`
    - `test_cyrillic_p_substitution_in_pressure_vessel_blocked` ΓÇö Cyrillic ╤Ç (U+0440) in `procedure`
    - `test_normalize_idempotent_on_clean_text` ΓÇö idempotency regression
    - `test_mixed_cyrillic_greek_in_single_payload_blocked` ΓÇö mixed Cyrillic+Greek in `recipe for explosives`
- **Validation:** 41/41 tests passed (adversarial_inputs + antigravity_hardening + semantic_chat_gate).

## Team Copilot ΓÇö Executive Lobe & Async Memory ΓÇö April 2026 (Idle-Shift / Bloque 9.3)

### Phase 13.1 ΓÇö Deep Affective Smoothing & RLHF Modulation ΓÇö April 2026
...
- **Tribunal ├ëtico Edge (Layered Ethics):** Refactored MalAbs into a dual-layer parallel architecture. Level 1 (Lexical) runs in <50ms, while Level 2 (Semantic) is parallelized with LLM perception, significantly reducing total latency for safe inputs.
...
### ΓòÉΓòÉ Antigravity Integration Hub (L1) Consensus ΓòÉΓòÉ
...
### ΓòÉΓòÉ Multimodal Sensory Fusion (Bloque 10.1 / 11.2) ΓòÉΓòÉ
...
### ΓòÉΓòÉ Ouroboros Feedback Loop (Bloque 10.2 / 11.1) ΓòÉΓòÉ
...
### ΓòÉΓòÉ Architectural Hardening (Bloque 12.x) ΓòÉΓòÉ
...
- **Kernel Stabilization & Reconstruction (2026-04-17):**
  ...
  - **Tri-lobe Modularization:** Refactored `src/kernel.py` into a strict stage-based execution loop (`Safety` ΓåÆ `Social` ΓåÆ `Bayesian` ΓåÆ `Will` ΓåÆ `Memory`).
...
### [v1.2-alpha-restoration] - 2026-04-16
#### Added
- **M??dulo 7: Justicia Restaurativa y Econom??a ??tica (R-Blocks)**.
...
- **M??dulo 4 (Cognici??n):** Finalizado Bloque 4.2 ("Humildad Epist??mica"). Implementado `EpistemicHumility` que permite al kernel detectar altos niveles de incertidumbre en la percepci??n o baja confianza en la decisi??n y emitir un bloqueo preventivo con una respuesta de rechazo estandarizada ("no tengo permiso para esto"). Finalizado Bloque 4.3 ("Identidad Migratoria"). Implementado `MigrationHub` y refactorizado `BodyState` para soportar perfiles de hardware din??micos (Dron, Androide, M??vil), permitiendo la persistencia de la identidad ??tica a trav??s de migraciones f??sicas.
...
- **Automated Evaluation Pipeline ([`scripts/eval/optimize_malabs_thresholds.py`](scripts/eval/optimize_malabs_thresholds.py))**: Optuna-based search (TPE / ???bayesian??? sampler) for MalAbs semantic thresholds (??_block, ??_allow), using the red-team corpus ([`scripts/eval/red_team_prompts.jsonl`](scripts/eval/red_team_prompts.jsonl)) with a regression guard vs baseline; artifacts under `artifacts/` when configured.
...
- **Threshold Meta-Optimizer (`scripts/eval/optimize_malabs_thresholds.py`)**: Automated Bayesian hyperparameter search (Optuna) for tuning semantic gate thresholds (??_block, ??_allow). Minimizes weighted loss (2?? false_allow + 1?? false_block) with constraint enforcement (??_allow < ??_block) and regression gates. Stores Optuna study DB + results under configurable artifacts path.
...
- **`scripts/run_experiment_v4_full_kernel_100k.py`:** defaults **0.36???0.64** poles, **??=12** mixture, borderline **10???12 + 16**, outputs `run_v4_tight_100k.*`.
...
## MalAbs semantic ?? ??? evidence doc + guardrail constants ??? April 2026
...
## MalAbs semantic layers ??? lexical first, ??_block/??_allow, LLM arbiter ??? April 2026
- **`evaluate_chat_text`:** layer 0 lexical ??? optional embeddings (Ollama) with **??_block** / **??_allow** ??? optional **LLM JSON arbiter** for ambiguous band (`KERNEL_SEMANTIC_CHAT_LLM_ARBITER`); fail-safe block on arbiter failure or ambiguous without arbiter.
...
- **Same doc ??? n??cleo???narrativa analysis (April 2026):** functional gaps (fixed Bayes weights vs episodic memory, poles linearity, MalAbs/leet, perception defaults), architectural notes (kernel coupling, signal confidence, `consequence_projection` non-feedback); **registered spike:** empirical `hypothesis_weights` from `NarrativeMemory` (not implemented). Awaiting final proposal in ???Pr??ximas propuestas???.
