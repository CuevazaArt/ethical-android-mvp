...
- **SP-P1-02 follow-up — `KERNEL_SENSOR_INPUT_STRICT`:** Optional strict validation for merged WebSocket `sensor` payloads (`validate_sensor_dict_strict`, `SensorSnapshot.from_dict(..., strict=True)`, `snapshot_from_layers` reads env); chat returns `error=sensor_payload_invalid`. [`sensor_contracts.py`](src/modules/sensor_contracts.py), [`chat_server.py`](src/chat_server.py), [`tests/test_sensor_contracts.py`](tests/test_sensor_contracts.py), [`PROPOSAL_SENSOR_FUSION_NORMALIZATION.md`](docs/proposals/PROPOSAL_SENSOR_FUSION_NORMALIZATION.md), [`KERNEL_ENV_POLICY.md`](docs/proposals/KERNEL_ENV_POLICY.md), [`.env.example`](.env.example).
