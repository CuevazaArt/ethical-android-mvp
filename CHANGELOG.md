...
- **Bloque 0.1.6 / Pragmatismo V4.0 — `merge_sensor_hints` anti-poison:** `src/modules/sensor_contracts.py` — `_clamp01` maps non-finite inputs to `0.5`; `stability_score` low-stability nudge uses an explicit finite-`float` guard (excludes `bool`). Test: `tests/test_sensor_contracts.py::test_merge_clamps_nonfinite_incoming_signal_values`. **Validation: GitHub Actions** (`pytest tests/`). Plan: `docs/proposals/PLAN_WORK_DISTRIBUTION_TREE.md` (0.1.6).
