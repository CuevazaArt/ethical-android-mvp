...
### Cursor Team Updates
- **V14 `scripts/swarm_sync.py`:** Session handoff now appends `schema: swarm_sync_v1` JSON to `KERNEL_SWARM_SYNC_LOG` (default `data/swarm_sync_log.jsonl`, matching `.env.example`), embeds `v14_session_sync_record` (`sha256_short`) + optional `git_commit_short` and `resolve_llm_mode`, supports `--dry-run` and `--log`; still updates cerebellum `swarm_sync_tail` via `append_swarm_sync_event`. Tests: `tests/test_swarm_sync_script.py`.
