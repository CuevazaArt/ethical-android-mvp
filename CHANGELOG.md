...
- **`kernel_lobes/__init__.py` pipeline diagram (Session 3):** Added ASCII flow diagram documenting the full tri-lobe pipeline — `PerceptiveLobe.observe()` → `LimbicEthicalLobe.judge()` → `ExecutiveLobe.formulate_response()` — including data types at each step (`SemanticState`, `EthicalSentence`, final `str`) and `CerebellumNode` daemon thread shown separately. Implements the recommendation from `PROPOSAL_COPILOT_HEMISPHERE_CRITIQUE.md`. `ruff` clean; 13/13 kernel-lobe tests pass.
- **`.gitignore` cleanup (Session 3 — Instrucción 2.2):** Removed two stale entries: `*.pyo` (Python 2 artifact — Python 3.12+ only generates `__pycache__/*.pyc`, never `.pyo`) and `get-pip.py` (one-off bootstrap script that does not live in any project). No tracked files affected; `__pycache__/` and `*.pyc` continue to cover all Python 3 bytecode.
- **`LLMPerception.confidence` property + mypy gaps (Session 3):** Added `@property confidence` to `LLMPerception` in `llm_layer.py` returning `max(0.0, 1.0 - self.risk)` — resolves the mypy `attr-defined` error in `perception_lobe.py:68` where `perception.confidence` was used but the attribute did not exist on the dataclass. Annotated `overall_risk` as `Literal["critical", "high", "medium", "low"]` in `external_audit_framework.py` to fix mypy's type-narrowing failure on the ternary chain. Added `issue_restorative_reparation` stub to `MockDAO` in `mock_dao.py` (mirrors the `DAOOrchestrator` signature, resolving `union-attr` on `self.dao`). Fixed `vision_adapter.py`: annotated `categories: list[str]` and introduced local `net` variable to avoid `attr-defined` errors on `self.model` when mypy infers it as `None`. All 1185 tests pass; `mypy src/` reports **Success: no issues found in 182 source files**.
