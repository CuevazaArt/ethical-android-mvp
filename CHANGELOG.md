...
- **Field test readiness + ADR 0017 honesty:** [`docs/proposals/PROPOSAL_FIELD_TEST_PLAN.md`](docs/proposals/PROPOSAL_FIELD_TEST_PLAN.md) §12 — readiness table for F1 first smartphone LAN session; [`docs/adr/0017-smartphone-sensor-relay-bridge.md`](docs/adr/0017-smartphone-sensor-relay-bridge.md) status **Accepted** with checklist reconciled to code (pending: `RealTimeBridge` token-bucket, optional focused tests). [`.env.example`](.env.example) documents `KERNEL_FIELD_*`. [`docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md`](docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md) — field-test gating milestone. [`docs/proposals/PROPOSAL_MULTIMODAL_CHARM_ENGINE_DIGEST.md`](docs/proposals/PROPOSAL_MULTIMODAL_CHARM_ENGINE_DIGEST.md) — link to Cursor playbook in §7.
- **Cursor playbook + LLM track — charm digest cross-links:** [`docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md`](docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md) §6 primary references and §9 execution slice list [`PROPOSAL_MULTIMODAL_CHARM_ENGINE_DIGEST.md`](docs/proposals/PROPOSAL_MULTIMODAL_CHARM_ENGINE_DIGEST.md); [`docs/proposals/PROPOSAL_LLM_INTEGRATION_TRACK.md`](docs/proposals/PROPOSAL_LLM_INTEGRATION_TRACK.md) §4 related docs + §5 changelog (presentation tier vs ethics core; no new gap ID).
- **Multimodal charm engine digest (P0):** [`docs/proposals/PROPOSAL_MULTIMODAL_CHARM_ENGINE_DIGEST.md`](docs/proposals/PROPOSAL_MULTIMODAL_CHARM_ENGINE_DIGEST.md) — maps conversational multimodal “charm” / UX stack to Ethos Kernel seams (`process_chat_turn`, LLM touchpoints, lobes, observability); critiques, guardrails (MalAbs, input trust, DAO/L0), phased roadmap; indexed in [`docs/proposals/README.md`](docs/proposals/README.md).
- **Cursor mission + backlog docs (G-04 alignment):** [`docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md`](docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md) §6–§9 — SP-P0-01 reflects closed G-04 (verbal/monologue/global default + `/health`); “next slice” retitled to current P1 maintenance; P2 milestone adds optional P0 Issue #2 pointer; research backlog distinguishes **legacy prefix rename** (deferred) from **G-04** (closed). [`MODEL_CRITICAL_BACKLOG.md`](docs/proposals/MODEL_CRITICAL_BACKLOG.md) row 2 updated; [`KERNEL_ENV_POLICY.md`](docs/proposals/KERNEL_ENV_POLICY.md) LLM index; [`docs/proposals/README.md`](docs/proposals/README.md) LLM track index (G-01…G-11); [`PROPOSAL_LLM_INTEGRATION_TRACK.md`](docs/proposals/PROPOSAL_LLM_INTEGRATION_TRACK.md) changelog line for Issue #7 / P2.
- **Claude playbook BI-P0-01 (`KERNEL_BAYESIAN_MODE` contract):** [`resolve_kernel_bayesian_mode()`](src/modules/bayesian_engine.py) + `ENV_KERNEL_BAYESIAN_MODE`; invalid env falls back to `disabled` with warning; `EthicalKernel` uses resolver at construction. Docs: [`CLAUDE_TEAM_PLAYBOOK_REAL_BAYESIAN_INFERENCE.md`](docs/proposals/CLAUDE_TEAM_PLAYBOOK_REAL_BAYESIAN_INFERENCE.md), [`KERNEL_ENV_POLICY.md`](docs/proposals/KERNEL_ENV_POLICY.md); [`kernel_env_operator.py`](src/validators/kernel_env_operator.py) classifies `KERNEL_BAYESIAN_*` under Poles/mixture; tests in [`tests/test_kernel_env_operator.py`](tests/test_kernel_env_operator.py); [`.env.example`](.env.example).
- **P2 polish — deploy verification + deprecation docs:** [`docs/deploy/COMPOSE_PRODISH.md`](docs/deploy/COMPOSE_PRODISH.md) checklist step **3b** (`/health` `observability.metrics_enabled` vs `/metrics`); [`docs/REPOSITORY_LAYOUT.md`](docs/REPOSITORY_LAYOUT.md) E2E section (no in-repo `landing/`, `src/static/` surfaces); [`KERNEL_ENV_POLICY.md`](docs/proposals/KERNEL_ENV_POLICY.md) §4 aligned with populated `DEPRECATION_ROADMAP` / `DEPRECATED_FLAGS`; [`tests/test_deprecation_warnings.py`](tests/test_deprecation_warnings.py) `test_deprecation_roadmap_has_no_extra_keys`; [`PLAN_IMMEDIATE_TWO_WEEKS.md`](docs/proposals/PLAN_IMMEDIATE_TWO_WEEKS.md) P2 status paragraph.
- **Issue #7 — `KERNEL_*` env policy closure:** [`KERNEL_ENV_POLICY.md`](docs/proposals/KERNEL_ENV_POLICY.md) **Implementation status** table (rules, CI, CLI); [`PLAN_IMMEDIATE_TWO_WEEKS.md`](docs/proposals/PLAN_IMMEDIATE_TWO_WEEKS.md) P1 Issue #7 marked closed for the two-week checklist; [`tests/test_env_policy.py`](tests/test_env_policy.py) `test_warn_logs_violations_without_raise`; [`CRITIQUE_ROADMAP_ISSUES.md`](docs/proposals/CRITIQUE_ROADMAP_ISSUES.md) Issue 7 delivered line; [`MODEL_CRITICAL_BACKLOG.md`](docs/proposals/MODEL_CRITICAL_BACKLOG.md) next-tier note; [`CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md`](docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md) §9 next milestone → P2 + G-04 research.
- **Issue #4 — packaging / decision-chain drift guard:** [`tests/test_core_packaging_boundary_docs.py`](tests/test_core_packaging_boundary_docs.py) now asserts optional extras (`runtime`, `theater`, `ml`, `vectors`, `llm`, `tuning`, `all-optional`, `dev`), empty `theater` marker, `runtime` includes FastAPI, and `project.urls.Documentation` points at [`CORE_DECISION_CHAIN.md`](docs/proposals/CORE_DECISION_CHAIN.md). [`docs/adr/0001-packaging-core-boundary.md`](docs/adr/0001-packaging-core-boundary.md) links the test; [`CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md`](docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md) §9 updated.
- **Issue #3 — empirical pilot default fixture:** [`scripts/run_empirical_pilot.py`](scripts/run_empirical_pilot.py) now defaults to [`tests/fixtures/labeled_scenarios.json`](tests/fixtures/labeled_scenarios.json) (canonical Issue 3 dataset); [`tests/fixtures/empirical_pilot/scenarios.json`](tests/fixtures/empirical_pilot/scenarios.json) remains a slim schema with the same 21 kernel outcomes (`--fixture` to select). [`EMPIRICAL_METHODOLOGY.md`](docs/proposals/EMPIRICAL_METHODOLOGY.md) — **Implementation status** table + run instructions; [`MODULE_IMPACT_AND_EMPIRICAL_GAP.md`](docs/proposals/MODULE_IMPACT_AND_EMPIRICAL_GAP.md) cross-link. Tests: [`tests/test_empirical_pilot.py`](tests/test_empirical_pilot.py), [`tests/test_empirical_pilot_runner.py`](tests/test_empirical_pilot_runner.py).
- **ADR 0015 / Integration sprint I1–I5 — closed in docs + I2 emission:** [`PROPOSAL_INTEGRATION_SPRINT_I1_I5.md`](docs/proposals/PROPOSAL_INTEGRATION_SPRINT_I1_I5.md) and [`docs/adr/0015-interteam-integration-sprint.md`](docs/adr/0015-interteam-integration-sprint.md) record **landed** status on integration hubs; [`EthicalKernel._emit_kernel_weights_updated`](src/kernel.py) publishes [`EVENT_KERNEL_WEIGHTS_UPDATED`](src/modules/kernel_event_bus.py) when hierarchical feedback composition changes mixture weights; [`feedback_trust_weight()`](src/modules/weight_authority.py) for payload trust. Tests: [`tests/test_kernel_event_bus.py`](tests/test_kernel_event_bus.py), [`tests/test_integration_sprint_i1_i5.py`](tests/test_integration_sprint_i1_i5.py). [`CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md`](docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md) §9 — **next slice:** Issue **#3** empirical pilot / labeled scenarios, then **#4** packaging boundary (existing doc tests).
- **SP-P1-02 follow-up — `KERNEL_SENSOR_INPUT_STRICT`:** Optional strict validation for merged WebSocket `sensor` payloads (`validate_sensor_dict_strict`, `SensorSnapshot.from_dict(..., strict=True)`, `snapshot_from_layers` reads env); chat returns `error=sensor_payload_invalid`. [`sensor_contracts.py`](src/modules/sensor_contracts.py), [`chat_server.py`](src/chat_server.py), [`tests/test_sensor_contracts.py`](tests/test_sensor_contracts.py), [`tests/test_chat_server.py`](tests/test_chat_server.py) (`test_websocket_sensor_payload_invalid_when_strict`), [`CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md`](docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md), [`PROPOSAL_SENSOR_FUSION_NORMALIZATION.md`](docs/proposals/PROPOSAL_SENSOR_FUSION_NORMALIZATION.md), [`KERNEL_ENV_POLICY.md`](docs/proposals/KERNEL_ENV_POLICY.md), [`.env.example`](.env.example).
- **Coordination (L0 → L1):** Relay from **Juan (L0)** via Cursor — collaboration workflow and docs should stay **continuously updated** and **more efficient** over time; Antigravity (L1) should steer integration/pulse/tooling accordingly without weakening governance. Recorded in [`docs/collaboration/L0_WORKFLOW_CONTINUOUS_IMPROVEMENT_RELAY.md`](docs/collaboration/L0_WORKFLOW_CONTINUOUS_IMPROVEMENT_RELAY.md); linked from [`MERGE_AND_HUB_DECISION_TREE.md`](docs/collaboration/MERGE_AND_HUB_DECISION_TREE.md) and [`MULTI_OFFICE_GIT_WORKFLOW.md`](docs/collaboration/MULTI_OFFICE_GIT_WORKFLOW.md).
- **Issue #3 empirical dataset (`labeled_scenarios.json`):** Expanded **`harness: batch`** rows to **`batch_id` 1–21** (aligned with [`tests/fixtures/empirical_pilot/scenarios.json`](tests/fixtures/empirical_pilot/scenarios.json)); fixed batch-06 illustrative label (`ask_for_clarification`); scenarios 17–19 use **`expected_decision: null`** (mapping-only). Updated [`tests/test_labeled_scenarios.py`](tests/test_labeled_scenarios.py), [`docs/proposals/EMPIRICAL_METHODOLOGY.md`](docs/proposals/EMPIRICAL_METHODOLOGY.md), [`docs/proposals/EMPIRICAL_PILOT_METHODOLOGY.md`](docs/proposals/EMPIRICAL_PILOT_METHODOLOGY.md), [`experiments/validation/README.md`](experiments/validation/README.md).
- **SP-P1-02 sensor fusion normalization:** [`docs/proposals/PROPOSAL_SENSOR_FUSION_NORMALIZATION.md`](docs/proposals/PROPOSAL_SENSOR_FUSION_NORMALIZATION.md) documents canonical `SensorSnapshot.from_dict` → `merge_sensor_hints_into_signals` + multimodal doubt; [`sensor_contracts.py`](src/modules/sensor_contracts.py) docstring link; [`tests/test_sensor_contracts.py`](tests/test_sensor_contracts.py) `test_merge_suppresses_audio_stress_nudges_when_multimodal_doubt`.
- **Tri-lobe limbic + executive + SP-P1-01:** [`LimbicEthicalLobe.judge`](src/kernel_lobes/limbic_lobe.py) maps `SemanticState.timeout_trauma` to `social_tension_locus` / `applied_trauma_weight`; [`ExecutiveLobe`](src/kernel_lobes/executive_lobe.py) surfaces tension in stub output when `> 0.05`. [`SensorAdapter`](src/kernel_lobes/sensor_adapter.py) / stubs + [`PROPOSAL_SENSOR_ADAPTER_CONTRACT.md`](docs/proposals/PROPOSAL_SENSOR_ADAPTER_CONTRACT.md). Integration tests [`tests/test_kernel_lobes_stack.py`](tests/test_kernel_lobes_stack.py) (orchestrator + probe + sensor doubles).
- **Tri-lobe PerceptiveLobe async I/O:** [`src/kernel_lobes/perception_lobe.py`](src/kernel_lobes/perception_lobe.py) uses optional ``httpx.AsyncClient`` when ``KERNEL_PERCEPTIVE_LOBE_PROBE_URL`` is set; transport errors map to ``SemanticState.timeout_trauma``. [`CorpusCallosumOrchestrator.shutdown`](src/kernel.py) closes the client. [`KERNEL_ENV_POLICY.md`](docs/proposals/KERNEL_ENV_POLICY.md) and [`kernel_env_operator.py`](src/validators/kernel_env_operator.py) index the key; [`tests/test_perceptive_lobe.py`](tests/test_perceptive_lobe.py).
- **Cursor line / tri-lobe:** Recorded L1 request response in [`docs/proposals/PROPOSAL_CURSOR_PERCEPTIVE_LOBE_BOUNDARIES.md`](docs/proposals/PROPOSAL_CURSOR_PERCEPTIVE_LOBE_BOUNDARIES.md) (vision + soft robotics stay on perceptual path; pointers to `kernel_lobes` and ADR 0002 follow-ups). Linked from [`CURSOR_REQUEST_HEMISPHERE_REFACTOR.md`](CURSOR_REQUEST_HEMISPHERE_REFACTOR.md). Refreshed [`docs/proposals/DEVELOPMENT_STATUS_20260415.md`](docs/proposals/DEVELOPMENT_STATUS_20260415.md) (full suite bar; obsolete hang/merge notes).
...
- **G-05 burst-cancel concurrency smoke:** [`src/modules/llm_cancel_burst.py`](src/modules/llm_cancel_burst.py), [`scripts/eval/run_burst_cancel_smoke.py`](scripts/eval/run_burst_cancel_smoke.py), [`tests/test_llm_cancel_burst_operational.py`](tests/test_llm_cancel_burst_operational.py); [`scripts/eval/run_llm_vertical_tests.py`](scripts/eval/run_llm_vertical_tests.py) includes the operational pytest; [`ADR 0002`](docs/adr/0002-async-orchestration-future.md) closes the former “burst stress tests” gap; [`PROPOSAL_LLM_INTEGRATION_TRACK.md`](docs/proposals/PROPOSAL_LLM_INTEGRATION_TRACK.md) G-05 notes + [`PROPOSAL_LLM_VERTICAL_ROADMAP.md`](docs/proposals/PROPOSAL_LLM_VERTICAL_ROADMAP.md) phase 5 targets updated.
- **G-10 integration gate (burst aligned):** [`scripts/eval/run_cursor_integration_gate.py`](scripts/eval/run_cursor_integration_gate.py) includes [`tests/test_llm_cancel_burst_operational.py`](tests/test_llm_cancel_burst_operational.py); [`CURSOR_CROSS_TEAM_INTEGRATION_GATE.md`](docs/collaboration/CURSOR_CROSS_TEAM_INTEGRATION_GATE.md) regression list updated.
...
- **G-04 closure — global default API + resolver wiring:** Implemented `ENV_LLM_GLOBAL_DEFAULT_POLICY` and `raw_global_default_policy()` in [`llm_touchpoint_policies.py`](src/modules/llm_touchpoint_policies.py) (fixes `GET /health` imports); monologue resolver honors the same knob when valid. Invalid legacy env values now fall through to the global default per [`PROPOSAL_LLM_TOUCHPOINT_DEGRADATION_MATRIX.md`](docs/proposals/PROPOSAL_LLM_TOUCHPOINT_DEGRADATION_MATRIX.md). [`PROPOSAL_LLM_INTEGRATION_TRACK.md`](docs/proposals/PROPOSAL_LLM_INTEGRATION_TRACK.md) marks G-04 closed; [`tests/test_llm_touchpoint_policies.py`](tests/test_llm_touchpoint_policies.py) covers cross-touchpoint validation.
