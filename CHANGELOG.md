...
### Cursor Team Updates
- **V14 `scripts/swarm_sync.py`:** CLI `--msg`, `--block`, `--author`, `--ticket`, `--log` (JSONL path, default `data/swarm_sync_log.jsonl`), `--dry-run`; stdout line includes `llm_resolved_mode`, `KERNEL_LLM_CLOUD_DISABLED` snapshot, optional `somatic_biographic_anchor` from disk, and nested `v14` digests from `v14_session_sync_record` (biography `canonical_summary` prefix from `KERNEL_CEREBELLUM_BIOGRAPHY_PATH`); without `--dry-run`, appends via `append_swarm_sync_event` and appends the same row to JSONL. Tests: `tests/test_swarm_sync_script.py`.
