...
async def test_nomad_vision_flow_injects_into_next_turn(monkeypatch):
    ...
    consumer.adapter.infer = lambda *args, **kwargs: VisionInference(
        primary_label="test_object", confidence=0.99
    )
    ...
    for _ in range(50):
        if (
            consumer.latest_inference is not None
            and consumer.latest_inference.primary_label == "test_object"
        ):
            found = True
            break
