...
def test_vitality_hint_only_when_critical():
    assert vitality_communication_hint(VitalityAssessment(0.5, 0.05, False, 40.0, 80.0, False, False)) == ""
    h = vitality_communication_hint(VitalityAssessment(0.02, 0.05, True, 40.0, 80.0, False, False))
    assert "vitality" in h.lower() or "power" in h.lower() or "operational" in h.lower()


def test_to_public_dict():
    d = VitalityAssessment(0.04, 0.05, True, 40.0, 80.0, False, False).to_public_dict()
