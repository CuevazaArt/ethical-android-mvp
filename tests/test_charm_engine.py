...
import pytest
from src.modules.charm_engine import CharmEngine, StylizedResponse
...
@pytest.fixture
def profile():
    return InteractionProfile(agent_id="test_user", circle=TrustCircle.UCHI_CERCANO, intimacy_level=0.8)
...
def test_direct_response_at_low_trust(engine, user_tracker):
    ...
    soto_profile = InteractionProfile(agent_id="stranger", circle=TrustCircle.SOTO_NEUTRO, intimacy_level=0.1)
    res = engine.apply(
