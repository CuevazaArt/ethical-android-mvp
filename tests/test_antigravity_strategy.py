...
def test_strategic_mission_lifecycle():
    ...
    alignment_low = kernel.strategist.evaluate_strategic_alignment(
        action_desc="I will read a book about birds.", active_mission_id=mission.id
    )
...
def test_self_generated_mission_alignment():
    ...
    assert (
        "Active Missions: Understand human empathy" in kernel.strategist.active_missions_summary()
    )
