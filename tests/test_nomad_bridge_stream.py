...
def test_vitality_impact_from_nomad_sensor():
    ...
    snap = SensorSnapshot(accelerometer_jerk=0.9, core_temperature=45.0, battery_level=0.9)
...
def test_nomad_charm_feedback(nomad_client):
    ...
    bridge.charm_feedback_queue.put_nowait(
        {"intimacy": 0.5, "warmth": 0.8, "playfulness": 0.2, "tension": 0.0}
    )
...
def test_nomad_ws_rejects_message_over_utf8_cap(monkeypatch, nomad_client):
    monkeypatch.setenv("KERNEL_NOMAD_WS_MAX_MESSAGE_BYTES", "80")
    bridge = get_nomad_bridge()
    huge = json.dumps({"type": "telemetry", "payload": {"x": "y" * 200}})
    assert len(huge.encode("utf-8")) > 80
    with nomad_client.websocket_connect("/ws/nomad") as websocket:
...
def test_nomad_ws_rejects_non_object_root(nomad_client):
    bridge = get_nomad_bridge()
    with nomad_client.websocket_connect("/ws/nomad") as websocket:
...
def test_nomad_ws_rejects_payload_not_dict(nomad_client):
    bridge = get_nomad_bridge()
    with nomad_client.websocket_connect("/ws/nomad") as websocket:
        websocket.send_json({"type": "telemetry", "payload": "not-a-dict"})
        time.sleep(0.05)
    assert bridge.telemetry_queue.empty()


def test_nomad_vision_rejects_decode_larger_than_cap(monkeypatch, nomad_client):
    """After decode, reject blobs larger than KERNEL_NOMAD_MAX_VISION_DECODED_BYTES."""
    monkeypatch.setenv("KERNEL_NOMAD_MAX_VISION_DECODED_BYTES", "10")
    raw = b"x" * 20
