...
- **Environment variable catalog (all `KERNEL_*` / Ollama / related keys read from `src/`):** [`docs/ENV_VAR_CATALOG.md`](docs/ENV_VAR_CATALOG.md) — includes §3.1 RLHF reward model and MalAbs→Bayesian bridge (`KERNEL_RLHF_*`); copy-from [`.env.example`](.env.example) for local shells.
