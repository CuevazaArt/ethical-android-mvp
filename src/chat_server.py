...
async def nomad_bridge_ws_handler(websocket: WebSocket) -> None:
    ...
    Bloque 22.2: after the socket is accepted, emits ``SYNC_IDENTITY`` with
    :class:`~src.kernel_lobes.models.GestaltSnapshot` fields and narrative tail
    so the PWA can repaint PAD state across flaky mobile reconnects.
    """
    from .modules.nomad_bridge import get_nomad_bridge
...
def build_sync_identity_ws_message(kernel: EthicalKernel) -> dict[str, Any]:
    ...
    identity_pub = _identity_state_public_dict(kernel)
    try:
