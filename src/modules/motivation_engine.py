...
class MotivationEngine:
    ...
    def __init__(self):
        self.drives: Dict[DriveType, InternalDrive] = {
            ...
            DriveType.SOCIAL_REPAIR: InternalDrive(DriveType.SOCIAL_REPAIR, value=0.0, growth_rate=0.0),
            DriveType.MAINTENANCE: InternalDrive(DriveType.MAINTENANCE, value=0.0, growth_rate=0.01),
            DriveType.COMMUNITY_AID: InternalDrive(DriveType.COMMUNITY_AID, value=0.5, growth_rate=0.01),
        }
    ...
    def get_proactive_actions(self) -> List[Dict[str, Any]]:
        ...
        active_drives = sorted(
            [d for d in self.drives.values() if d.value >= d.threshold],
        ...
        if top.type == DriveType.CURIOSITY:
            return [
                {"name": "investigate_uncertain_context", "description": "Seek clarity on current ambiguous scenario", "impact": 0.3},
                {"name": "ask_for_clarification", "description": "Engage user to resolve epistemic doubt", "impact": 0.2}
            ]
        elif top.type == DriveType.SOCIAL_REPAIR:
            return [
                {"name": "offer_courtesy_gesture", "description": "De-escalate perceived social tension", "impact": 0.4},
                {"name": "reparation_proposal", "description": "Propose action to compensate for previous friction", "impact": 0.5}
            ]
        elif top.type == DriveType.MAINTENANCE:
            return [
                {"name": "request_rest_cycle", "description": "Enter low-power state to conserve energy", "impact": 0.1},
                {"name": "self_diagnostic", "description": "Run internal integrity check", "impact": 0.1}
            ]
        elif top.type == DriveType.COMMUNITY_AID:
            return [
                {"name": "advance_mission_task", "description": "Proactively work on active strategic mission", "impact": 0.6}
            ]
