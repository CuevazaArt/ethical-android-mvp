...
from .vision_adapter import from_env_vision_adapter, VisionInference
...
def vision_worker(input_queue: mp.Queue, output_queue: mp.Queue, stop_event: mp.Event, shm_name: str, shm_shape: tuple, shm_dtype: np.dtype):
    ...
    def start(self):
        ...
        self._process = mp.Process(
            ...
            args=(self._input_q, self._output_q, self._stop_event, 
                  self._shm.name, self._frame_shape, self._dtype),
            daemon=True
        )
        self._process.start()
