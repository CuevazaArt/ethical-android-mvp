...
class MobileNetV2Adapter(VisionAdapter):
    ...
    def load_model(self, path: str = None) -> None:
        ...
        try:
            ...
            self._is_ready = True
            print(f"[Vision] MobileNetV2 loaded successfully on {actual_device}.")

        except ImportError:
            print("[Vision] Warning: torch/torchvision not found. MobileNetV2Adapter will run in MOCK mode.")
            self._is_ready = False
    ...
    def infer(self, frame: Any) -> VisionInference:
        ...
        try:
            ...
            raw_scores = {self.categories[idx.item()]: float(c) for c, idx in zip(top5_conf, top5_idx)}
