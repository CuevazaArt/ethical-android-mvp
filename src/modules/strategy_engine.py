...
class ExecutiveStrategist:
    ...
    def create_mission(self, title: str, origin: MissionOrigin, steps: list[str] = None, priority: float = 0.5) -> Mission:
        if len(self.missions) >= self.max_missions:
            completed = [m.id for m in self.missions.values() if m.status in (MissionStatus.SUCCESS, MissionStatus.FAILED)]
            if completed:
    ...
    def evaluate_strategic_alignment(self, action_desc: str, active_mission_id: str = None) -> float:
        """
        ...
        if not target_mission:
            active = sorted([m for m in self.missions.values() if m.status == MissionStatus.ACTIVE], key=lambda x: -x.priority)
            if not active:
                return 0.0
        ...
        remaining_steps = [s for s in target_mission.steps if s not in target_mission.completed_steps]
        if remaining_steps:
            if any(word in action_l for s in remaining_steps for word in s.lower().split() if len(word) > 3):
                match_score += 0.6
    ...
    def active_missions_summary(self) -> str:
        ...
        summary = "Active Missions: " + " | ".join([f"{m.title} ({len(m.completed_steps)}/{len(m.steps)})" for m in active])
        return summary
