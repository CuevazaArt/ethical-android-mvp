    ...
    def transmit(self, projection: AffectProjection) -> None:
        ...
        try:
            ...
            payload = {
                ...
                "visuals": {"color": hex_color, "pulse_s": pulse_duration, "scale": glow_size},
            }
            ...
            if (
                self.bridge
                and hasattr(self.bridge, "charm_feedback_queue")
                and self.bridge.charm_feedback_queue
            ):
                ...
                if latency > 2.0:
                    _log.debug(
                        "AffectVesselRelay: transmitted orb_update (Arch: %s, lat: %.2fms)",
                        projection.dominant_archetype_id,
                        latency,
                    )
            else:
                _log.warning(
                    "AffectVesselRelay: Nomad Bridge or queue unavailable. Expression dropped."
                )
