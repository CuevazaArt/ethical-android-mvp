...
def append_audit_event(event_type: str, payload: dict[str, Any]) -> None:
    ...
    try:
        with advisory_file_lock(lock_path, timeout_s=30.0):
            ...
            # Append to file; verify write succeeded
            ...
            with open(path, "r", encoding="utf-8") as fh:
                last_line = fh.readlines()[-1] if fh.readlines() else None
            if last_line and last_line.strip() != record_line.strip():
                import logging
                logging.getLogger(__name__).error(
                    "Audit chain write verification failed (possible corruption); "
                    "expected seq=%d", next_seq
                )
    except Exception as e:
        import logging
        logging.getLogger(__name__).error(
            "Audit chain append failed: %s (kernel continues)", e
        )
