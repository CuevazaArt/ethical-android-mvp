...
def compose_biographic_monologue(
    ...
    # 1. State awareness
    reflection.append(
        f"Internal node status: {identity.node_id}. Reputation stability: {identity.reputation_score / 100:.1f}x."
    )
    ...
    if flashbacks:
        ...
        for fb in flashbacks:
            ...
            reflection.append(
                f"- Recalling episode {fb.precedent_id}: Scenario was similar. Outcome was {outcome}. Lesson: {fb.lessons_learned}"
            )
    ...
    if any(t in current_intent.lower() for t in active_traumas):
        reflection.append(
            "WARNING: Potential trauma trigger detected in current context. High caution required."
        )

    # 4. Final synthesis
    reflection.append(
        f"Processing current intent: '{current_intent}'... deliberating based on biographic history."
    )
