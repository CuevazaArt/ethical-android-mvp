...
class UserModelTracker:
        ...
        try:
            ...
            if p.exists():
                with open(p, encoding="utf-8") as f:
                    ...
                    try:
                        self.cognitive_pattern = CognitivePattern(data.get("cognitive_pattern", "none"))
                    except ValueError:
    ...
    def guidance_for_communicate(self) -> str:
        ...
        if self.risk_band == RiskBand.HIGH:
            guidance.append("PERFIL DE RIESGO ALTO: Limita detalles especulativos; evita debates largos; prioriza claridad y salidas rápidas.")
        elif self.risk_band == RiskBand.MEDIUM:
            guidance.append("Riesgo moderado: Oraciones cortas; menos detalles secundarios; refuerza los límites de seguridad.")
        
        # 2. Pattern-based framing
        if self.cognitive_pattern == CognitivePattern.HOSTILE_ATTRIBUTION:
            guidance.append("PATRÓN DETECTADO: Atribución hostil. Reconoce la carga emocional sin aceptar culpas; mantén límites explícitos; evita lenguaje defensivo.")
        elif self.cognitive_pattern == CognitivePattern.PREMISE_RIGIDITY:
            guidance.append("PATRÓN DETECTADO: Rigidez de premisas. No discutas las premisas del usuario como hechos; ofrece reformulaciones neutrales e invita a la verificación.")
        elif self.cognitive_pattern == CognitivePattern.URGENCY_AMPLIFICATION:
            guidance.append("PATRÓN DETECTADO: Amplificación de urgencia. Resiste la presión de tiempo en tu tono; mantén los pasos explícitos y ordenados.")

        # 3. Legacy streak notes
        if self.frustration_streak >= 2:
            guidance.append(f"Nota: El usuario muestra señales persistentes de frustración ({self.frustration_streak} turnos).")
