...
**Maturation disclaimer:** some **historical names** (e.g. `BayesianEngine` — a **weighted mixture**, not full Bayes; see [`weighted_ethics_scorer.py`](src/modules/weighted_ethics_scorer.py), [ADR 0009](docs/adr/0009-ethical-mixture-scorer-naming.md)) and **governance mocks** (DAO) are **infrastructure placeholders** while runtime, audit trails, and tests mature — toward **honest labeling**, **calibrated weights**, and **clear criteria** for stronger safety and real governance experiments. **Weaknesses / bottlenecks** (sync LLM in worker threads vs async cancellation, inference fragility, MockDAO limits): [`docs/WEAKNESSES_AND_BOTTLENECKS.md`](docs/WEAKNESSES_AND_BOTTLENECKS.md). **Consolidated** issue backlog (two external reviews, redundant items merged): [`docs/proposals/CRITIQUE_ROADMAP_ISSUES.md`](docs/proposals/CRITIQUE_ROADMAP_ISSUES.md); **~2-week maintainer plan** (P0/P1/P2, reproduction checklists): [`docs/proposals/PLAN_IMMEDIATE_TWO_WEEKS.md`](docs/proposals/PLAN_IMMEDIATE_TWO_WEEKS.md); web overview: [Roadmap](https://mosexmacchinalab.com/roadmap). **Forward-looking hardening (non-binding):** [`docs/proposals/PRODUCTION_HARDENING_ROADMAP.md`](docs/proposals/PRODUCTION_HARDENING_ROADMAP.md) — includes synthesis of external reviews: strengths, criticisms (value vs redundancy), and **conclusions for improvements** (short / medium / long term); proposal round closed as doc; follow-up via issues/ADRs.
...
**Core decision chain & packaging (Issue 4):** [`docs/proposals/CORE_DECISION_CHAIN.md`](docs/proposals/CORE_DECISION_CHAIN.md) (mermaid + table: who sets `final_action`); [`docs/adr/0001-packaging-core-boundary.md`](docs/adr/0001-packaging-core-boundary.md), [`docs/adr/0002-async-orchestration-future.md`](docs/adr/0002-async-orchestration-future.md) (thread offload + optional turn timeout / dedicated pool; full async LLM TBD). **At a glance:**
...
**Governance narrative (Issue 6):** [`docs/proposals/GOVERNANCE_MOCKDAO_AND_L0.md`](docs/proposals/GOVERNANCE_MOCKDAO_AND_L0.md) — MockDAO limits, **L0** as in-repo constitution, L1/L2 vs votes; **simulation honesty** (no chain product, QV assumptions): [`docs/proposals/MOCK_DAO_SIMULATION_LIMITS.md`](docs/proposals/MOCK_DAO_SIMULATION_LIMITS.md), [`contracts/README.md`](contracts/README.md); public page [blockchain-dao](https://mosexmacchinalab.com/blockchain-dao).
...
| Risky combos / lab-only | (ad-hoc mixes) | [Unsupported combinations](docs/proposals/KERNEL_ENV_POLICY.md#3-unsupported-or-lab-only-combinations); [strict rules](docs/proposals/KERNEL_ENV_POLICY.md#2-explicitly-rejected-combinations-when-kernel_env_validationstrict) (`KERNEL_ENV_VALIDATION`) |
...
Optional env: `CHAT_HOST`, `CHAT_PORT`, `LLM_MODE`, `USE_LOCAL_LLM`, `KERNEL_VARIABILITY`, `KERNEL_ADVISORY_INTERVAL_S` (background drive telemetry per WebSocket session; see [RUNTIME_CONTRACT.md](docs/proposals/RUNTIME_CONTRACT.md)), `KERNEL_CHAT_TURN_TIMEOUT` (seconds — async deadline per turn; JSON `chat_turn_timeout` on expiry; does not cancel sync Ollama; see [ADR 0002](docs/adr/0002-async-orchestration-future.md)), `KERNEL_CHAT_THREADPOOL_WORKERS` (dedicated thread pool for chat offload; `0` = default), `KERNEL_CHAT_JSON_OFFLOAD` (default on — build WebSocket JSON off the asyncio loop; see [PROPOSAL_SYNC_KERNEL_ASYNC_ASGI_BRIDGE.md](docs/proposals/PROPOSAL_SYNC_KERNEL_ASYNC_ASGI_BRIDGE.md)), `KERNEL_API_DOCS` (set to `1` to expose `/docs`, `/redoc`, `/openapi.json`; **default off** so LAN binds do not leak API schemas), `KERNEL_CHAT_EXPOSE_MONOLOGUE` (set to `0` to redact `monologue` in WebSocket JSON and skip LLM monologue embellishment), `KERNEL_CHAT_INCLUDE_HOMEOSTASIS` (set to `0` to omit `affective_homeostasis` — σ/strain/PAD advisory UX only).

**Chat server settings (schema):** [`src/chat_settings.py`](src/chat_settings.py) documents and loads a growing subset of these flags via Pydantic (`CHAT_HOST`, `CHAT_PORT`, `KERNEL_API_DOCS`, `KERNEL_VARIABILITY`, `LLM_MODE`, `KERNEL_CHAT_INCLUDE_MALABS_TRACE`, `KERNEL_CHAT_TURN_TIMEOUT`, `KERNEL_CHAT_THREADPOOL_WORKERS`, `KERNEL_CHAT_JSON_OFFLOAD`). WebSocket JSON may include **`malabs_trace`** — ordered atomic step ids for MalAbs (lexical + semantic + optional arbiter) when `KERNEL_CHAT_INCLUDE_MALABS_TRACE` is on (default on); set to `0` to omit.
...
**Perception circuit (metacognitive doubt):** unrelated to lighthouse — when `KERNEL_PERCEPTION_CIRCUIT=1` (default), repeated stressed LLM perception parses can set **top-level** `metacognitive_doubt` on WebSocket JSON and force `gray_zone` tone. See `src/modules/perception_circuit.py` and [KERNEL_ENV_POLICY.md](docs/proposals/KERNEL_ENV_POLICY.md) (perception row).

**Generative third-way candidates (v9.2, opt-in):** `KERNEL_GENERATIVE_ACTIONS=1` appends traceable candidates (`source=generative_proposal`, unique `proposal_id`) on heavy chat turns when dilemma-like phrasing is detected (or optional `KERNEL_GENERATIVE_TRIGGER_CONTEXTS=1` for high-stakes `suggested_context`). With **`KERNEL_GENERATIVE_LLM=1`**, perception JSON from the api/ollama path may include `generative_candidates` (parsed instead of templates when valid). `KERNEL_GENERATIVE_ACTIONS_MAX` caps how many (default 2). Same MalAbs → mixture scorer path as built-in actions (`WeightedEthicsScorer`; alias `BayesianEngine`). WebSocket `decision` includes `chosen_action_source` and optional `proposal_id`. See `src/modules/generative_candidates.py`.
...
(Absolute Evil), to weighted ethical impact scoring (mixture over hypotheses; not full Bayes), the "weakness pole" that
