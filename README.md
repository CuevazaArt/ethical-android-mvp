...
**Maturation disclaimer:** some **names** (e.g. “Bayesian” scoring) and **governance mocks** (DAO) are **infrastructure placeholders** while runtime, audit trails, and tests mature — toward **honest labeling**, **calibrated weights**, and **clear criteria** for stronger safety and real governance experiments. **Consolidated** issue backlog (two external reviews, redundant items merged): [`docs/proposals/CRITIQUE_ROADMAP_ISSUES.md`](docs/proposals/CRITIQUE_ROADMAP_ISSUES.md); web overview: [Roadmap](https://mosexmacchinalab.com/roadmap). **Forward-looking hardening (non-binding):** [`docs/proposals/PRODUCTION_HARDENING_ROADMAP.md`](docs/proposals/PRODUCTION_HARDENING_ROADMAP.md) — includes synthesis of external reviews: strengths, criticisms (value vs redundancy), and **conclusions for improvements** (short / medium / long term); proposal round closed as doc; follow-up via issues/ADRs.
...
| Perception / sensors / vitality | `KERNEL_SENSOR_*`, `KERNEL_MULTIMODAL_*`, `KERNEL_VITALITY_*`, optional `KERNEL_PERCEPTION_UNCERTAINTY_DELIB` / `KERNEL_PERCEPTION_UNCERTAINTY_MIN` | [Flag families](docs/proposals/KERNEL_ENV_POLICY.md#1-flag-families-mental-model) |
...
**Operations:** supported demo **env bundles** — [`src/runtime_profiles.py`](src/runtime_profiles.py); strategy and roadmap — [docs/proposals/ESTRATEGIA_Y_RUTA.md](docs/proposals/ESTRATEGIA_Y_RUTA.md).
...
**Relational / v7 (optional JSON toggles, default on):** `KERNEL_CHAT_INCLUDE_USER_MODEL`, `KERNEL_CHAT_INCLUDE_CHRONO`, `KERNEL_CHAT_INCLUDE_PREMISE`, `KERNEL_CHAT_INCLUDE_TELEOLOGY` — set to `0` to omit `user_model`, `chronobiology`, `premise_advisory`, `teleology_branches`. Base doc: [docs/proposals/PROPUESTA_EVOLUCION_RELACIONAL_V7.md](docs/proposals/PROPUESTA_EVOLUCION_RELACIONAL_V7.md).
...
**Semantic MalAbs layers (opt-in, ADR 0003):** `KERNEL_SEMANTIC_CHAT_GATE=1` enables **embedding similarity** (Ollama `POST /api/embeddings`) **after** the **lexical** substring pass — order: lexical → embeddings → optional LLM arbiter. **θ_block** / **θ_allow:** `KERNEL_SEMANTIC_CHAT_SIM_BLOCK_THRESHOLD` (default `0.82`; legacy `KERNEL_SEMANTIC_CHAT_SIM_THRESHOLD` if block unset) and `KERNEL_SEMANTIC_CHAT_SIM_ALLOW_THRESHOLD` (default `0.45`). Between them, **ambiguous** text can go to **`KERNEL_SEMANTIC_CHAT_LLM_ARBITER=1`** using the same text backend as chat (`src/modules/semantic_chat_gate.py`, `EthicalKernel` passes `llm._text_backend`). Embed model: `KERNEL_SEMANTIC_CHAT_EMBED_MODEL=nomic-embed-text`; `OLLAMA_BASE_URL` shared with LLM. If Ollama is down, only lexical MalAbs applies. Details: [`docs/proposals/MALABS_SEMANTIC_LAYERS.md`](docs/proposals/MALABS_SEMANTIC_LAYERS.md).
...
├── docs/
│   ├── proposals/        # Theory ↔ implementation, RUNTIME_*, roadmaps, PROPUESTA_* / English drafts, experimental notes (single folder)
...
**Roadmap (foundation — discussion notes + phased code):** Guardian Angel — [PROPUESTA_ANGEL_DE_LA_GUARDIA.md](docs/proposals/PROPUESTA_ANGEL_DE_LA_GUARDIA.md). **v9** — [PROPUESTA_CAPACIDAD_AMPLIADA_V9.md](docs/proposals/PROPUESTA_CAPACIDAD_AMPLIADA_V9.md). **v10** — diplomacy, skills, somatic markers, metaplan (MVP) — [PROPUESTA_ESTRATEGIA_OPERATIVA_V10.md](docs/proposals/PROPUESTA_ESTRATEGIA_OPERATIVA_V10.md). **V11** — distributed justice / DAO escalation (Phase 1 in code) — [PROPUESTA_JUSTICIA_DISTRIBUIDA_V11.md](docs/proposals/PROPUESTA_JUSTICIA_DISTRIBUIDA_V11.md). **V12** — moral infrastructure hub — [PROPUESTA_ESTADO_ETOSOCIAL_V12.md](docs/proposals/PROPUESTA_ESTADO_ETOSOCIAL_V12.md) (registry) + **[UNIVERSAL_ETHOS_AND_HUB.md](docs/proposals/UNIVERSAL_ETHOS_AND_HUB.md)** (unified vision ↔ code). Phases **1–3** + stubs: `moral_hub`, `GET /constitution`, `GET /dao/governance`, drafts, DAO votes, `deontic_gate`, `ml_ethics_tuner`, `reparation_vault`, `nomad_identity`. **Nomadic HAL** — [PROPUESTA_CONCIENCIA_NOMADA_HAL.md](docs/proposals/PROPUESTA_CONCIENCIA_NOMADA_HAL.md), `hardware_abstraction`, `existential_serialization`. Env: see V12 doc (`KERNEL_DEONTIC_GATE`, `KERNEL_ML_ETHICS_TUNER_LOG`, `KERNEL_REPARATION_VAULT_MOCK`, `KERNEL_CHAT_INCLUDE_NOMAD_IDENTITY`, …).

**Experimental paper (same lineage, Spanish draft):** expected phenomena when coupling PAD + prototype mixing to the kernel; *color* / *flavor* as metaphors; testable hypotheses for future runs — [docs/proposals/PAPER_AFECTO_FENOMENOS_Y_HIPOTESIS.md](docs/proposals/PAPER_AFECTO_FENOMENOS_Y_HIPOTESIS.md).
