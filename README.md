...
**Semantic chat gate (opt-in, ADR 0003):** `KERNEL_SEMANTIC_CHAT_GATE=1` enables an **embedding similarity** pass via Ollama `POST /api/embeddings` **before** substring MalAbs (`src/modules/semantic_chat_gate.py`). Default embed model: `KERNEL_SEMANTIC_CHAT_EMBED_MODEL=nomic-embed-text`; cosine threshold `KERNEL_SEMANTIC_CHAT_SIM_THRESHOLD` (default `0.82`). Uses the same `OLLAMA_BASE_URL` as the LLM. If Ollama is unreachable or the embed model is missing, the gate returns no verdict and **substring MalAbs still applies** — no silent failure of the conservative lists.
