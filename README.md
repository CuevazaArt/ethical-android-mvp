...
**Maturation disclaimer:** some **names** (e.g. “Bayesian” scoring) and **governance mocks** (DAO) are **infrastructure placeholders** while runtime, audit trails, and tests mature — toward **honest labeling**, **calibrated weights**, and **clear criteria** for stronger safety and real governance experiments. **Weaknesses / bottlenecks** (sync core vs async server, inference fragility, MockDAO limits): [`docs/WEAKNESSES_AND_BOTTLENECKS.md`](docs/WEAKNESSES_AND_BOTTLENECKS.md). **Consolidated** issue backlog (two external reviews, redundant items merged): [`docs/proposals/CRITIQUE_ROADMAP_ISSUES.md`](docs/proposals/CRITIQUE_ROADMAP_ISSUES.md); **~2-week maintainer plan** (P0/P1/P2, reproduction checklists): [`docs/proposals/PLAN_IMMEDIATE_TWO_WEEKS.md`](docs/proposals/PLAN_IMMEDIATE_TWO_WEEKS.md); web overview: [Roadmap](https://mosexmacchinalab.com/roadmap). **Forward-looking hardening (non-binding):** [`docs/proposals/PRODUCTION_HARDENING_ROADMAP.md`](docs/proposals/PRODUCTION_HARDENING_ROADMAP.md) — includes synthesis of external reviews: strengths, criticisms (value vs redundancy), and **conclusions for improvements** (short / medium / long term); proposal round closed as doc; follow-up via issues/ADRs.
