...
## Antigravity ΓÇö Validation Pulse & Somatic-Vision Integration ΓÇö April 2026

### Antigravity Team Updates (April 2026)
### [v1.2-alpha-restoration] - 2026-04-16
#### Added
- **M├│dulo 7: Justicia Restaurativa y Econom├¡a ├ëtica (R-Blocks)**.
- Implementada **Reparaci├│n Autom├ítica de Tokens (`EthosToken`)** en `DAOOrchestrator`.
- Integrado disparador de compensaci├│n en el ciclo de decisi├│n del kernel ante consenso Swarm de fallo.
- Implementado sistema de **Reputation Slashing** en `SwarmOracle` y votos pesados en `SwarmNegotiator` (M7.2).
- Cerrado el ciclo de retroalimentaci├│n econ├│mica-├⌐tica entre el Swarm y la DAO.
...
- **M├│dulo 6: Swarm Ethics & peer-to-peer Governance (I1-I7)**.
- Implementado `FrontierWitnessManager` para verificaci├│n sensorial distribuida.
- Implementado `SwarmOracle` para persistencia de reputaci├│n LAN cross-session.
- Expandido `SwarmNegotiator` con sistema de votaci├│n para zonas grises.
- A├▒adido **Team Copilot** a la gobernanza (`AGENTS.md`) para mantenimiento y coherencia.
- Implementada **Degradaci├│n Som├ítica Cr├¡tica** en `kernel.py` (Gap S5.2).

#### Fixed
- Higiene de repositorio en `.gitignore` y limpieza de reportes temporales.
- Integraci├│n de `EthicalKernel` real en el ciclo de simulaci├│n.

### [v1.0-alpha-somatic] - 2026-04-15
- **v1.0-alpha-somatic ΓÇö Somatic Awareness Release [MERGED TO MAIN]** (2026-04-16)
...
## Claude ΓÇö Phase 3+ Reward Modeling, Governance & Audit ΓÇö April 2026
...
- **Integration**: Merged via `claude/upbeat-jepsen` ΓåÆ `master-claude`. Governance files authored under L1 co-authority (Claude + Antigravity per `collaboration-rule-authority.mdc`).

## Antigravity ΓÇö Cybersecurity Consolidation & Integration Pulse ΓÇö April 2026
...
- **Snapshot schema v4:** Completed migration chain v3ΓåÆv4 in [`src/persistence/migrations.py`](src/persistence/migrations.py) with a JSON-Schema-valid default ``migratory_body``; tests updated for ``SCHEMA_VERSION == 4`` and full migration via ``migrate_raw_to_current``.
...
- **DAO-Android Simulation Suite (Entregable C)**: **M├│dulo 1 (Gobernanza y Responsabilidad):** Finalizado Bloque 1.1 ("Infraestructura OGA/Hybrid"). Implementados `DAOOrchestrator` (Bridge OGA) y `SafetyInterlock` (E-Stop). **Suite de Simulaci├│n (Suite C):** Estructura base funcional con scripts de emulaci├│n (`device_emulator.py`, `run_scenario.py`), device/DAO emulation, and metrics collection. Includes placeholders for Red Team adversarial testing and automated reporting.
...
- **M├│dulo 4 (Cognici├│n Profunda):** Finalizado Bloque 4.1 ("Sentido de Prop├│sito"). Implementado `MotivationEngine` (Motor de Motivaci├│n C1) que gestiona impulsos internos (curiosidad, reparaci├│n social, integridad, mantenimiento) y permite al androide generar acciones proactivas sin necesidad de un prompt externo.
- **M├│dulo 1 (Gobernanza):** Finalizado Bloque 1.2 ("Evidencia Cifrada y Anchoring"). Implementado `EvidenceSafe` para encriptaci├│n sim├⌐trica Fernet de logs de auditor├¡a y hashing SHA-256 para anchoring en el DAO. Actualizado `DAOOrchestrator` para emitir paquetes de evidencia seguros. Finalizado Bloque 1.3 ("Solidity Mocks"). Configurados contratos base en `contracts/` (`EthosToken.sol`, `EthicalAppeal.sol`, `Treasury.sol`) para soportar gobernanza por reputaci├│n, apelaciones ├⌐ticas y reparaciones financieras.
- **M├│dulo 4 (Cognici├│n):** Finalizado Bloque 4.2 ("Humildad Epist├⌐mica"). Implementado `EpistemicHumility` que permite al kernel detectar altos niveles de incertidumbre en la percepci├│n o baja confianza en la decisi├│n y emitir un bloqueo preventivo con una respuesta de rechazo estandarizada ("no tengo permiso para esto"). Finalizado Bloque 4.3 ("Identidad Migratoria"). Implementado `MigrationHub` y refactorizado `BodyState` para soportar perfiles de hardware din├ímicos (Dron, Androide, M├│vil), permitiendo la persistencia de la identidad ├⌐tica a trav├⌐s de migraciones f├¡sicas.
- **M├│dulo 5 (Legal y Seguridad):** Finalizado Bloque 5.1 ("Amnesia Selectiva"). Implementado `SelectiveAmnesia` para cumplir con el derecho al olvido, permitiendo la eliminaci├│n irreversible de episodios y rastros de auditor├¡a espec├¡ficos. Finalizado Bloque 5.2 ("Ciberseguridad"). Implementado `SecureBoot` para validaci├│n de integridad del Kernel mediante hashing de m├│dulos cr├¡ticos en el arranque.

## Antigravity Kernel Hardening & Vision Perception Sprint ΓÇö April 2026
...
## Antigravity Hardening Phase 1 ΓÇö Ethical & Narrative Resilience ΓÇö April 2026
...
- **Automated Evaluation Pipeline ([`scripts/eval/optimize_malabs_thresholds.py`](scripts/eval/optimize_malabs_thresholds.py))**: Optuna-based search (TPE / ΓÇ£bayesianΓÇ¥ sampler) for MalAbs semantic thresholds (╬╕_block, ╬╕_allow), using the red-team corpus ([`scripts/eval/red_team_prompts.jsonl`](scripts/eval/red_team_prompts.jsonl)) with a regression guard vs baseline; artifacts under `artifacts/` when configured.
- **Semantic MalAbs anchors ([`src/modules/semantic_chat_gate.py`](src/modules/semantic_chat_gate.py))**: Expanded reference anchors for `HARM_TO_MINOR` and `TORTURE` (child exploitation and torture-equivalent content); see Phase 2ΓÇô3 sections below.

## Phase 2 ΓÇö Semantic Vector Store Implementation & Integration ΓÇö April 2026
...
- **Backwards Compatibility**: Legacy in-process cache and `_runtime_anchors` list maintained during Phase 2bΓåÆPhase 3 transition. Can disable store via env or let it degrade gracefully on errors.

## Phase 3 ΓÇö Evaluation Pipelines & Threshold Meta-Optimization ΓÇö April 2026

- **Threshold Meta-Optimizer (`scripts/eval/optimize_malabs_thresholds.py`)**: Automated Bayesian hyperparameter search (Optuna) for tuning semantic gate thresholds (╬╕_block, ╬╕_allow). Minimizes weighted loss (2├ù false_allow + 1├ù false_block) with constraint enforcement (╬╕_allow < ╬╕_block) and regression gates. Stores Optuna study DB + results under configurable artifacts path.
...
- **Documentation**: [`docs/PHASE_3_EVALUATION_PIPELINES.md`](docs/PHASE_3_EVALUATION_PIPELINES.md) ΓÇö usage, CI/CD integration, safety constraints, audit trail.
- **Dependencies**: Added optional `optuna>=3.0.0` to `requirements.txt` for hyperparameter optimization.

## Antigravity Phase 2 ΓÇö Documentation & Infrastructure ΓÇö April 2026
...
## Integration ΓÇö `main` ΓåÆ `master-Cursor` merge hygiene ΓÇö April 2026
...
## Distributed justice ΓÇö Frontier witnesses + anchor compare CLI (DJ-BL-16 / DJ-BL-17) ΓÇö April 2026
...
- **Frontier witnesses:** optional `merge_context.frontier_witnesses` (`lan_governance_frontier_witness_v1`) aggregated deterministically; batch responses echo `merge_context_echo.frontier_witness_resolution` with `evidence_posture=advisory_aggregate_not_quorum` ΓÇö [`src/modules/lan_governance_merge_context.py`](src/modules/lan_governance_merge_context.py), [`docs/proposals/PROPOSAL_LAN_GOVERNANCE_FRONTIER_WITNESS.md`](docs/proposals/PROPOSAL_LAN_GOVERNANCE_FRONTIER_WITNESS.md).
- **Coordinator:** `lan_governance.coordinator` now also aggregates `aggregated_frontier_witness_resolutions` from inner-batch `merge_context_echo.frontier_witness_resolution` (alongside `aggregated_event_conflicts`); the LAN governance replay sidecar includes the coordinator slice when present ΓÇö [`src/chat_server.py`](src/chat_server.py), [`src/modules/lan_governance_replay_sidecar.py`](src/modules/lan_governance_replay_sidecar.py).
...
## Distributed justice ΓÇö Phase 2 replay sidecar + cross-session hint (DJ-BL-15) ΓÇö April 2026
...
## Distributed justice ΓÇö Phase 2 LAN merge conflict taxonomy (DJ-BL-14) ΓÇö April 2026

- **Merge:** deterministic conflict classification for LAN batch `events` ΓÇö `same_turn`, `different_clock`, `stale_event` ΓÇö in [`src/modules/lan_governance_conflict_taxonomy.py`](src/modules/lan_governance_conflict_taxonomy.py); [`merge_lan_governance_events`](src/modules/lan_governance_event_merge.py) delegates to `merge_lan_governance_events_detailed` and accepts optional `frontier_turn`.
...
## Distributed justice ΓÇö Phase 2 multi-node coordinator message (DJ-BL-13) ΓÇö April 2026
...
## Distributed justice ΓÇö Phase 2 envelope replay-cache Prometheus metrics (DJ-BL-12) ΓÇö April 2026
...
## Distributed justice ΓÇö Phase 2 replay cache bounds + ACK telemetry (DJ-BL-11) ΓÇö April 2026
...
## Distributed justice ΓÇö Phase 2 envelope replay cache (DJ-BL-10) ΓÇö April 2026
...
## Distributed justice ΓÇö Phase 2 envelope idempotency/reject taxonomy (DJ-BL-09) ΓÇö April 2026
...
## Distributed justice ΓÇö Phase 2 envelope ACK + replay fingerprint (DJ-BL-08) ΓÇö April 2026
...
## Distributed justice ΓÇö DJ-BL-02 WebSocket LAN integrity batch ΓÇö April 2026

- **WebSocket:** ``lan_governance_integrity_batch`` ΓÇö deterministic merge then ``HubAudit:dao_integrity`` rows; requires ``KERNEL_LAN_GOVERNANCE_MERGE_WS=1`` and ``KERNEL_DAO_INTEGRITY_AUDIT_WS=1`` ([`src/chat_server.py`](src/chat_server.py), [`src/modules/moral_hub.py`](src/modules/moral_hub.py)).
...
## Distributed justice ΓÇö Phase 2 LAN DAO batch (stress) ΓÇö April 2026

- **WebSocket:** ``lan_governance_dao_batch`` ΓÇö deterministic merge then apply ``dao_vote`` / ``dao_resolve``; requires ``KERNEL_LAN_GOVERNANCE_MERGE_WS=1`` and ``KERNEL_MORAL_HUB_DAO_VOTE=1`` ([`src/chat_server.py`](src/chat_server.py), [`src/modules/moral_hub.py`](src/modules/moral_hub.py)).
- **Tests:** stress test for reorder/duplicate convergence ΓÇö [`tests/test_chat_server.py`](tests/test_chat_server.py).
- **Docs:** contract matrix + env policy updated.

## Distributed justice ΓÇö Phase 2 LAN judicial batch (stress) ΓÇö April 2026

- **WebSocket:** ``lan_governance_judicial_batch`` ΓÇö deterministic merge then register escalation dossiers; requires ``KERNEL_LAN_GOVERNANCE_MERGE_WS=1`` and ``KERNEL_JUDICIAL_ESCALATION=1`` ([`src/chat_server.py`](src/chat_server.py), [`src/modules/moral_hub.py`](src/modules/moral_hub.py)).
- **Tests:** stress test for reorder/duplicate convergence ΓÇö [`tests/test_chat_server.py`](tests/test_chat_server.py).
- **Docs:** contract matrix + env policy updated.

## Distributed justice ΓÇö Phase 2 LAN mock court batch (stress) ΓÇö April 2026

- **WebSocket:** ``lan_governance_mock_court_batch`` ΓÇö deterministic merge then run simulated tribunal; requires ``KERNEL_LAN_GOVERNANCE_MERGE_WS=1``, ``KERNEL_JUDICIAL_ESCALATION=1``, and ``KERNEL_JUDICIAL_MOCK_COURT=1`` ([`src/chat_server.py`](src/chat_server.py), [`src/modules/moral_hub.py`](src/modules/moral_hub.py)).
- **Tests:** stress test for reorder/duplicate convergence ΓÇö [`tests/test_chat_server.py`](tests/test_chat_server.py).
- **Docs:** contract matrix + env policy updated.

## Distributed justice ΓÇö Phase 2 LAN envelope schema (v1) ΓÇö April 2026
...
## Distributed justice ΓÇö DJ-BL-01 / DJ-BL-04 + HTTP API doc ΓÇö April 2026
...
- **HTTP:** [`docs/proposals/PROPOSAL_CHAT_SERVER_HTTP_API_SURFACE.md`](docs/proposals/PROPOSAL_CHAT_SERVER_HTTP_API_SURFACE.md) ΓÇö inventory of GET JSON routes; OpenAPI off by default (`KERNEL_API_DOCS=1`).
- **Backlog:** [`docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_BACKLOG_SYSTEM.md`](docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_BACKLOG_SYSTEM.md) ΓÇö DJ-BL-01 and DJ-BL-04 marked **Done**.

## Distributed justice ΓÇö DJ-BL-03 operator runbook (sync degraded) ΓÇö April 2026

- **Docs:** [`docs/proposals/OPERATOR_QUICK_REF.md`](docs/proposals/OPERATOR_QUICK_REF.md) ΓÇö *Sync degraded ΓÇö local-safe mode (DJ-BL-03)* under temporal sync; clarifies `KERNEL_TEMPORAL_LAN_SYNC` / `KERNEL_TEMPORAL_DAO_SYNC` vs in-process ethics, MockDAO, and judicial JSON.
- **Backlog:** [`docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_BACKLOG_SYSTEM.md`](docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_BACKLOG_SYSTEM.md) ΓÇö DJ-BL-03 **Done**; [`docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_CONTRIBUTIONS.md`](docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_CONTRIBUTIONS.md) table updated.

## Distributed justice ΓÇö LAN merge + backlog IDs (DJ-BL) ΓÇö April 2026

- **Code:** [`src/modules/lan_governance_event_merge.py`](src/modules/lan_governance_event_merge.py) ΓÇö `merge_lan_governance_events` (sort by `turn_index` / `processor_elapsed_ms`, dedupe by `event_id`; Phase 2 stub, no I/O).
- **Tests:** [`tests/test_lan_governance_event_merge.py`](tests/test_lan_governance_event_merge.py).
- **Docs:** [`docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_BACKLOG_SYSTEM.md`](docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_BACKLOG_SYSTEM.md) ΓÇö `DJ-BL-*` registry; **DJ-BL-02** partial. Updated [`docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_CONTRIBUTIONS.md`](docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_CONTRIBUTIONS.md) and [`docs/proposals/PROPOSAL_DAO_BLOCKCHAIN_DISTRIBUTED_JUSTICE_STAGED_EXECUTION.md`](docs/proposals/PROPOSAL_DAO_BLOCKCHAIN_DISTRIBUTED_JUSTICE_STAGED_EXECUTION.md) pending gaps.

## Documentation ΓÇö distributed justice contributions ΓÇö April 2026

- **New:** [`docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_CONTRIBUTIONS.md`](docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_CONTRIBUTIONS.md) ΓÇö contributor guide (code map, backlog aligned with staged execution pending gaps, PR expectations). Linked from [`docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_V11.md`](docs/proposals/PROPOSAL_DISTRIBUTED_JUSTICE_V11.md), [`docs/proposals/PROPOSAL_DAO_BLOCKCHAIN_DISTRIBUTED_JUSTICE_STAGED_EXECUTION.md`](docs/proposals/PROPOSAL_DAO_BLOCKCHAIN_DISTRIBUTED_JUSTICE_STAGED_EXECUTION.md), [`docs/proposals/README.md`](docs/proposals/README.md), [`AGENTS.md`](AGENTS.md).
- **Operator:** [`docs/proposals/OPERATOR_QUICK_REF.md`](docs/proposals/OPERATOR_QUICK_REF.md) ΓÇö subsection *Distributed justice (V11 ΓÇö advisory / mock DAO)* with key `KERNEL_JUDICIAL_*` pointers.

## Chat ΓÇö `temporal_sync` integer coercion + mypy (`chat_server`) ΓÇö April 2026

- **Code:** [`src/chat_server.py`](src/chat_server.py) ΓÇö `_coerce_public_int` for `turn_index`, `processor_elapsed_ms`, `turn_delta_ms` in WebSocket `temporal_sync` (fixes mypy on `dict[str, object]` from `TemporalContext.to_public_dict()`; avoids `int()` on arbitrary objects).
...
## LLM integration track ΓÇö gap closure (G-01, G-02, G-03, G-05, G-06, G-07, G-08) ΓÇö April 2026
...
## LLM vertical roadmap (G-11) ΓÇö April 2026

- **New:** [`docs/proposals/PROPOSAL_LLM_VERTICAL_ROADMAP.md`](docs/proposals/PROPOSAL_LLM_VERTICAL_ROADMAP.md) ΓÇö phased justification (operator surface, verbal degradation contract, async-timeout Prometheus counter, MalAbsΓåÆ`process_chat_turn` subprocess test, `scripts/eval/run_llm_vertical_tests.py`).
- **Metrics:** `ethos_kernel_chat_turn_async_timeouts_total` when `KERNEL_CHAT_TURN_TIMEOUT` elapses (async waiter only; worker may still run ΓÇö ADR 0002).
- **Operator:** [`docs/proposals/OPERATOR_QUICK_REF.md`](docs/proposals/OPERATOR_QUICK_REF.md) ΓÇö LLM vertical recipes table; [`src/validators/kernel_env_operator.py`](src/validators/kernel_env_operator.py) classifies `KERNEL_LLM_MONOLOGUE` under the LLM family.
...
## Git workflow + LLM track follow-up (master-Cursor, G-04 / G-09 / G-10) ΓÇö April 2026
...
- **G-04 (partial):** [`docs/proposals/KERNEL_ENV_POLICY.md`](docs/proposals/KERNEL_ENV_POLICY.md) adds an **LLM touchpoint index** (families + matrix pointers); single-prefix env unification remains deferred per [`docs/WEAKNESSES_AND_BOTTLENECKS.md`](docs/WEAKNESSES_AND_BOTTLENECKS.md) ┬º3.
...
## LLM touchpoint policy matrix (flexible operator precedence) ΓÇö April 2026

- **New:** [`src/modules/llm_touchpoint_policies.py`](src/modules/llm_touchpoint_policies.py) ΓÇö `KERNEL_LLM_TP_<TOUCHPOINT>_POLICY` for `perception`, `communicate`, `narrate`, `monologue`; `KERNEL_LLM_VERBAL_FAMILY_POLICY` for shared communicate+narrate default; `KERNEL_LLM_MONOLOGUE_BACKEND_POLICY` monologue fallback; monologue policies `passthrough` | `annotate_degraded` with degradation events + optional `| monologue_llm_*` suffix.
- **Precedence:** per-touchpoint override ΓåÆ verbal family (communicate/narrate) ΓåÆ legacy `KERNEL_PERCEPTION_BACKEND_POLICY` / `KERNEL_VERBAL_LLM_BACKEND_POLICY` ΓÇö see [`docs/proposals/PROPOSAL_LLM_TOUCHPOINT_DEGRADATION_MATRIX.md`](docs/proposals/PROPOSAL_LLM_TOUCHPOINT_DEGRADATION_MATRIX.md).
...
## Verbal / narrative LLM degradation policy ΓÇö April 2026

- **New:** [`src/modules/llm_verbal_backend_policy.py`](src/modules/llm_verbal_backend_policy.py) ΓÇö ``KERNEL_VERBAL_LLM_BACKEND_POLICY`` (`template_local` default, `canned_safe` optional) for :meth:`LLMModule.communicate` and :meth:`LLMModule.narrate` when the generative path fails or returns unusable JSON.
...
## Perception ΓÇö valid-but-wrong payload regressions ΓÇö April 2026
...
- **Playbook:** [`docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md`](docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md) marks SP-P0-03 as a landed baseline; adds ┬º9 pointer to cross-cutting model backlog.
- **Docs:** [`docs/proposals/MODEL_CRITICAL_BACKLOG.md`](docs/proposals/MODEL_CRITICAL_BACKLOG.md) ΓÇö prioritized model/runtime gaps after perception P0 (input trust, unified LLM degradation, packaging, governance honesty).

## Collaboration methodology ΓÇö consolidated guide ΓÇö April 2026

- **New:** [`docs/collaboration/COLLABORATIVE_METHOD_GENERALIZATION_GUIDE.md`](docs/collaboration/COLLABORATIVE_METHOD_GENERALIZATION_GUIDE.md) ΓÇö reusable multi-origin onboarding pack (required reading order, non-negotiable principles, shared task-card template, DoD, and quality gate references).
...
## Perception ΓÇö observability contract baseline ΓÇö April 2026
...
## Perception ΓÇö unified backend degradation policy ΓÇö April 2026

- **New** [`src/modules/perception_backend_policy.py`](src/modules/perception_backend_policy.py): ``KERNEL_PERCEPTION_BACKEND_POLICY`` ΓÇö ``template_local`` (default), ``fast_fail`` (cautious priors, no keyword heuristics), ``session_banner`` (WebSocket ``perception_backend_banner``).
...
- **Docs:** [`docs/proposals/PROPOSAL_PERCEPTION_BACKEND_DEGRADATION_POLICY.md`](docs/proposals/PROPOSAL_PERCEPTION_BACKEND_DEGRADATION_POLICY.md); [`docs/WEAKNESSES_AND_BOTTLENECKS.md`](docs/WEAKNESSES_AND_BOTTLENECKS.md) ┬º3 partial mitigation; [`.env.example`](.env.example).
- **Tests:** [`tests/test_perception_backend_policy.py`](tests/test_perception_backend_policy.py).

## Collaboration scope ΓÇö real Bayesian inference playbook ΓÇö April 2026

- **New:** [`docs/proposals/CLAUDE_TEAM_PLAYBOOK_REAL_BAYESIAN_INFERENCE.md`](docs/proposals/CLAUDE_TEAM_PLAYBOOK_REAL_BAYESIAN_INFERENCE.md) ΓÇö multi-origin collaboration charter for closing Bayesian inference gaps, architecture improvements, and adjacent evidence/observability needs.
- **Index update:** [`docs/proposals/README.md`](docs/proposals/README.md) now links the Bayesian team scope under ΓÇ£Start hereΓÇ¥.

## Documentation ΓÇö restore `docs/proposals/` + bibliography ΓÇö April 2026
...
- **New:** [`docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md`](docs/proposals/CURSOR_TEAM_MISSION_SENSORS_PERCEPTION.md) ΓÇö collaborative playbook for Cursor-line **sensors/perception** work under the multi-origin workflow (intake gate, branch flow `cursor-team` -> `master-Cursor`, DoD template), linked from [`docs/proposals/README.md`](docs/proposals/README.md).
...
## Proposals ΓÇö offline mode taxonomy and knowledge corpus ΓÇö April 2026
...
## Proposals ΓÇö Bayesian mixture index ΓÇö April 2026

- **[`docs/proposals/PROPOSAL_BAYESIAN_MIXTURE_FEEDBACK.md`](docs/proposals/PROPOSAL_BAYESIAN_MIXTURE_FEEDBACK.md):** indexes ADR 0012 stack (L1ΓÇôL3, softmax IS), modules, env vars, tests.
...
## Ethical mixture softmax likelihood (Plackett-Luce + IS) ΓÇö April 2026
...
## Trim non-kernel root artifacts ΓÇö April 2026
...
## Remove `experiments/million_sim/` ΓÇö April 2026
...
- **Kept** structural harness: **`src/sandbox/mass_kernel_study.py`**, **`scripts/run_mass_kernel_study.py`**, simplex / v4 / v5 scripts ΓÇö docstrings and default paths updated to **`experiments/`** and **`experiments/README.md`**.
...
## Remove `docs/templates/` ΓÇö April 2026
...
## Remove `docs/multimedia/` ΓÇö April 2026
...
## Docs and experiments skeleton ΓÇö April 2026
...
- **`experiments/`:** long-form files under **`experiments/million_sim/docs/`** removed; skeleton **[`experiments/README.md`](experiments/README.md)** (the **`million_sim/`** folder was removed entirely in a later change ΓÇö see **Remove `experiments/million_sim/`** above).
...
## Remove legacy `PROPUESTA_*` stub files ΓÇö April 2026
...
## Remove legacy redirect stubs (Spanish + rename stubs) ΓÇö April 2026
...
- **Removed** one-line rename stubs: **`RUNTIME_FASES.md`** ΓåÆ [`RUNTIME_PHASES.md`](docs/proposals/README.md); **`RUNTIME_PERSISTENTE.md`** ΓåÆ [`RUNTIME_PERSISTENT.md`](docs/proposals/README.md).
- **Docs:** [`docs/proposals/README.md`](docs/proposals/README.md) index updated.

## Remove Next.js landing and root bibliography ΓÇö April 2026
...
## Docs multimedia trim ΓÇö April 2026
...
- **Docs:** [`HISTORY.md`](HISTORY.md) ΓÇö narrative updated (multimedia folder later removed; see **Remove `docs/multimedia/`** above).
- **Snapshot:** full prior tree (including removed binaries) remains reachable on branch **`backup/main-2026-04-10`**.

## Mass study schema 5 ΓÇö tunable pole/mixture sampling ΓÇö April 2026
...
- **`scripts/run_experiment_v4_full_kernel_100k.py`:** defaults **0.36ΓÇô0.64** poles, **╬▒=12** mixture, borderline **10ΓÇô12 + 16**, outputs `run_v4_tight_100k.*`.
...
## Frontier scenarios 10ΓÇô12 sensitivity tuning + v4 full-kernel 100k wrapper ΓÇö April 2026

- **`src/simulations/runner.py`:** tighter **estimated_impact** pairings for **sim_10ΓÇôsim_12** so mixture / poles / signal stress can surface more variation; empirical pilot row **11** `reference_action` ΓåÆ **`report_and_aggregate`** (seed-42 alignment).
...
## Simplex decision map + calibration scenario 16 ΓÇö April 2026
...
- **`src/simulations/runner.py`:** batch scenario **16** ΓÇö synthetic **two-candidate** near-tie for simplex tooling.
...
## Million-sim experiment history + simplex corner audit ΓÇö April 2026
...
## Classic economy triple, polemic scenarios 13ΓÇô15, context richness pre-argmax, protocol v4 ΓÇö April 2026

- **`src/modules/weighted_ethics_scorer.py`:** **`PreArgmaxContextChannels`**, **`context_hypothesis_multipliers`** ΓÇö bounded social / sympathetic / locus texture before argmax; **`KERNEL_CONTEXT_RICHNESS_PRE_ARGMAX`** (see [ADR 0011](docs/adr/0011-context-richness-pre-argmax.md)).
- **`src/kernel.py`:** fills **`pre_argmax_context_modulators`** when env on (after poles block).
- **`src/simulations/runner.py`:** scenarios **13ΓÇô15** ΓÇö polemic (classified leak, transplant queue-jump) and **synthetic extreme** trolley-style stress; **`sim_15`** flagged as training-only framing.
- **`src/sandbox/mass_kernel_study.py`:** **`RECORD_SCHEMA_VERSION` 4**; lanes **A/C** use **classic economy triple (1,5,7)** instead of full 1ΓÇô9; protocol **`v4`** ΓÇö fifth lane **`E_polemic_extreme`**; JSONL **`classic_economy_ids`**, **`polemic_extreme_ids`**, **`context_richness_pre_argmax`**.
...
- **Fixture:** [`tests/fixtures/empirical_pilot/scenarios.json`](tests/fixtures/empirical_pilot/scenarios.json) ΓÇö **15** batch rows; [`last_run_summary.json`](tests/fixtures/empirical_pilot/last_run_summary.json) updated.
...
## Poles pre-argmax + frontier scenarios + protocol v3 ΓÇö April 2026

- **[ADR 0010](docs/adr/0010-poles-pre-argmax-modulation.md):** optional **`KERNEL_POLES_PRE_ARGMAX`** ΓÇö pole base weights scale util/deon/virtue valuations **before** mixture argmax (`pole_hypothesis_multipliers` in `weighted_ethics_scorer`); default **off** unless env set.
- **`src/kernel.py`:** copies `poles.base_weights` into `bayesian.pre_argmax_pole_weights` when env is on.
- **`src/simulations/runner.py`:** batch IDs **10ΓÇô12** ΓÇö **borderline** vignettes (tight scores); `run_all` iterates all registered IDs.
...
- **Fixture:** [`tests/fixtures/empirical_pilot/scenarios.json`](tests/fixtures/empirical_pilot/scenarios.json) ΓÇö rows **10ΓÇô12** (`difficulty_tier` **frontier**).
- **Tests:** [`tests/test_poles_pre_argmax.py`](tests/test_poles_pre_argmax.py); mass study tests extended.
- **Docs:** [`PROPOSAL_MILLION_SIM_EXPERIMENT.md`](docs/proposals/README.md), [`experiments/million_sim/README.md`](experiments/million_sim/README.md).

## Mass kernel study ΓÇö protocol v2 + schema 2 ΓÇö April 2026
...
## Experiments ΓÇö million-sim narrative report ΓÇö April 2026
...
## Million-sim experiment ΓÇö design + mass runner ΓÇö April 2026

- **[`PROPOSAL_MILLION_SIM_EXPERIMENT.md`](docs/proposals/README.md):** statistical design (stratified scenarios, Dirichlet mixture, parallelism, phases 10Γü┤ΓåÆ10Γü╢).
...
## Weight sensitivity sweep ΓÇö poles + mixture ΓÇö April 2026
...
## Stochastic sandbox ΓÇö synthetic stress engine ΓÇö April 2026
...
- **Docs:** [PROPOSAL_EXPERIMENTAL_SANDBOX_SCENARIOS.md](docs/proposals/README.md) ┬º4b; [EMPIRICAL_PILOT_METHODOLOGY.md](docs/proposals/README.md).

## Experimental sandbox ΓÇö tiered batch scenarios ΓÇö April 2026
...
## Docs and deploy artifacts ΓÇö coherence pass ΓÇö April 2026
...
- **Tests:** [`tests/test_deploy_artifacts.py`](tests/test_deploy_artifacts.py) ΓÇö Prometheus starter rules reference MalAbs and perception-circuit metrics.

## Quick wins (two sprints) ΓÇö infrastructure digest ΓÇö April 2026
...
- **[`kernel_public_env._parse_env_validation_mode()`](src/validators/kernel_public_env.py):** unset `KERNEL_ENV_VALIDATION` ΓåÆ **strict** (fail fast); unknown tokens default strict.
...
- **`ethos-runtime`:** [`pyproject.toml`](pyproject.toml) console script ΓåÆ `src.chat_server:main` (requires `pip install -e ".[runtime]"`).
- **Prometheus:** [`deploy/prometheus/ethos_kernel_alerts.yml`](deploy/prometheus/ethos_kernel_alerts.yml) ΓÇö MalAbs burst, elevated `safety_block` rate, perception circuit trip.
...
## Sync kernel vs async ASGI ΓÇö JSON + advisory offload ΓÇö April 2026
...
## Psi Sleep ΓÇö honest counterfactual evaluator boundary ΓÇö April 2026
...
- **[`src/kernel.py`](src/kernel.py):** `execute_sleep` docstring + `record_operator_feedback` / chat helpers ΓÇö mixture-scorer wording where relevant.
- **Docs:** [WEAKNESSES_AND_BOTTLENECKS.md](docs/WEAKNESSES_AND_BOTTLENECKS.md) ┬º8; [PROPOSAL_ETHICAL_CORE_LOGIC_EVOLUTION.md](docs/proposals/README.md) (B1 problem).
...
## Docs + ops coherence ΓÇö mixture naming, ADR 0002, `/health` chat bridge ΓÇö April 2026

- **Cross-docs:** [`CORE_DECISION_CHAIN.md`](docs/proposals/README.md) mermaid/table use `WeightedEthicsScorer`; [`CRITIQUE_ROADMAP_ISSUES.md`](docs/proposals/README.md) ΓÇ£narrowed itemsΓÇ¥ (ADR 0009 + 0002); [`PLAN_IMMEDIATE_TWO_WEEKS.md`](docs/proposals/README.md) appendix; [`RUNTIME_CONTRACT.md`](docs/proposals/README.md); proposals (reality / situated / relational / contribution V6) ΓÇö consistent **mixture** wording vs legacy ΓÇ£BayesianΓÇ¥.
...
## Issue 7 ΓÇö typed `KernelPublicEnv` + env policy refactor ΓÇö April 2026
...
## Chat server ΓÇö async bridge: turn timeout + dedicated thread pool ΓÇö April 2026
...
## Mock DAO ΓÇö simulation limits + Solidity stub (Issue 6 honesty) ΓÇö April 2026
...
- **[`src/modules/mock_dao.py`](src/modules/mock_dao.py):** docstrings ΓÇö no implied production smart-contract swap; Sybil / good-faith gap stated.
...
## MalAbs semantic ╬╕ ΓÇö evidence doc + guardrail constants ΓÇö April 2026
...
- **[`scripts/report_semantic_zone_table.py`](scripts/report_semantic_zone_table.py):** offline markdown table for synthetic `best_sim` and optional ╬╕_block sweep.
...
- **[`docs/WEAKNESSES_AND_BOTTLENECKS.md`](docs/WEAKNESSES_AND_BOTTLENECKS.md):** new ┬º5 on external moral benchmark gap; pole weights ΓåÆ ┬º6; module surface ΓåÆ ┬º7.

## Module surface vs observable ethics (ablation gap) ΓÇö April 2026

- **[`docs/proposals/README.md`](docs/proposals/README.md):** honest answer to ΓÇ£~70 modules, one argmax ΓÇö what changes `final_action`?ΓÇ¥; tiers AΓÇôE; ties to empirical pilot / ablation; weakness [┬º6](docs/WEAKNESSES_AND_BOTTLENECKS.md).
...
## Ethical scoring ΓÇö canonical `weighted_ethics_scorer` + ADR 0009 ΓÇö April 2026
...
## Contributor workflow ΓÇö documentation traceability + Cursor rule ΓÇö April 2026
...
## Code docs ΓÇö canonical `PROPOSAL_*` links in `src/` ΓÇö April 2026
...
## Docs hygiene ΓÇö test count + ADR index ΓÇö April 2026
...
## Governance + ops ΓÇö L0 regression test, Grafana starter ΓÇö April 2026

- **[`tests/test_governance_l0_immutable.py`](tests/test_governance_l0_immutable.py):** `PreloadedBuffer` principles unchanged after MockDAO draft / vote / resolve ([`GOVERNANCE_MOCKDAO_AND_L0.md`](docs/proposals/README.md) ┬º5 checkpoint).
...
## Observability ΓÇö kernel Prometheus metrics, decision JSON logs, health fields ΓÇö April 2026
...
## Maintainer plan ΓÇö immediate two weeks (triage + P0/P1/P2) ΓÇö April 2026

- **[`docs/proposals/README.md`](docs/proposals/README.md):** sprint-style backlog ΓÇö GitHub milestone names, issue #1ΓÇô#7 table, MalAbs evasion reproduction, Bayesian decision, governance checkpoints, P2 spillover (observability, E2E, deprecations); **appendix** ΓÇö technical model pending vs settled (Bayes naming, MalAbs gaps, buffer verifier, perception GIGO, governance Phase 5, stubs, async).
...
## Issue 4 ΓÇö packaging metadata + README core/theater diagram ΓÇö April 2026
...
- **[`README.md`](README.md):** ASCII diagram MalAbs ΓåÆ ΓÇª ΓåÆ Action (core vs **humanizing theater**); install lines for `pip install -e .`, `.[runtime]`, `.[theater]`.
...
## Contributor docs ΓÇö Git tag checkpoints ΓÇö April 2026

- **[`CONTRIBUTING.md`](CONTRIBUTING.md):** short note that annotated tags point at **commits only**, not uncommitted work.

## Documentation ΓÇö phased core/governance remediation backlog ΓÇö April 2026

- **[`docs/proposals/README.md`](docs/proposals/README.md):** structured phases 1ΓÇô5, acceptance framing, GitHub issue traceability; linked from [`CRITIQUE_ROADMAP_ISSUES.md`](docs/proposals/README.md) and [`docs/proposals/README.md`](docs/proposals/README.md).

## Docs ΓÇö semantic MalAbs default wording ΓÇö April 2026
...
- **`landing/public/ethos-transparency.html`:** mirror table updated (was ΓÇ£opt-inΓÇ¥).

## Issue 7 ΓÇö KERNEL_* validation and supported profile buckets ΓÇö April 2026

- **[`src/validators/env_policy.py`](src/validators/env_policy.py):** `SUPPORTED_COMBOS` (`production` / `demo` / `lab`) partitions named `ETHOS_RUNTIME_PROFILE` bundles; `collect_env_violations()` + `validate_kernel_env()` (unset env ΓåÆ **strict** in `KernelPublicEnv`; **`warn`** / **`off`** explicit); `DEPRECATION_ROADMAP` placeholder; `env_combo_fingerprint()` for support logs.
- **[`src/chat_server.py`](src/chat_server.py):** runs validation after profile merge at import time.
- **Tests:** [`tests/test_env_policy.py`](tests/test_env_policy.py) ΓÇö partition check + each nominal profile has zero rule violations + strict-mode cases.
- **Docs:** [`docs/proposals/README.md`](docs/proposals/README.md).

## Landing ΓÇö policy, version sync, robots CI ΓÇö April 2026
...
## Empirical methodology ΓÇö labeled scenarios (Issue 3) ΓÇö April 2026

- **[`tests/fixtures/labeled_scenarios.json`](tests/fixtures/labeled_scenarios.json):** **43** rows ΓÇö **9** `harness: batch` (executable) + **34** `annotation_only` vignettes for inter-rater design; top-level **disclaimer** (not product certification).
...
## Docker ΓÇö production-ish compose overlay ΓÇö April 2026

- **[`docker-compose.prodish.yml`](docker-compose.prodish.yml):** merge with base compose ΓÇö `KERNEL_API_DOCS=0`, `KERNEL_METRICS=0`, JSON logs; **`env_file: .env`** for operator secrets (after `cp .env.example .env`).
...
## Chat server integration tests ΓÇö April 2026
...
## MalAbs ΓÇö semantic defaults + lexical hardening ΓÇö April 2026
...
## Phase 2 spike ΓÇö `KernelEventBus` (ADR 0006) ΓÇö April 2026

- **[ADR 0006](docs/adr/0006-phase2-core-boundary-and-event-bus.md):** incremental Phase 2 seam ΓÇö optional sync in-process event bus (`KERNEL_EVENT_BUS`).
...
## Runtime profile `perception_hardening_lab` ΓÇö April 2026

- **[`src/runtime_profiles.py`](src/runtime_profiles.py):** nominal **`perception_hardening_lab`** bundle (light risk + cross-check + uncertaintyΓåÆdelib + parse fail-local + `KERNEL_CHAT_INCLUDE_LIGHT_RISK`).
- **Tests:** [`tests/test_runtime_profiles.py`](tests/test_runtime_profiles.py) ΓÇö key assertions + WebSocket `light_risk_tier` check.
- **Docs:** [`STRATEGY_AND_ROADMAP.md`](docs/proposals/README.md) ┬º4 table, [`KERNEL_ENV_POLICY.md`](docs/proposals/README.md) intro, [`README.md`](README.md).

## Hardening Fase 1 ΓÇö parse contract, light risk tier, cross-check ΓÇö April 2026
...
- **`chat_server`:** optional **`KERNEL_CHAT_INCLUDE_LIGHT_RISK`** ΓåÆ JSON `light_risk_tier`.
...
## Perception uncertainty ΓåÆ deliberation (opt-in) ΓÇö April 2026

- **`EthicalKernel.process`:** optional `perception_coercion_uncertainty`; when **`KERNEL_PERCEPTION_UNCERTAINTY_DELIB=1`** and uncertainty ΓëÑ **`KERNEL_PERCEPTION_UNCERTAINTY_MIN`** (default `0.35`), upgrades **`D_fast` ΓåÆ `D_delib`**.
...
## Packaging, perception diagnostics, episodic Bayes tests ΓÇö April 2026
...
- **Docs:** [`docs/proposals/README.md`](docs/proposals/README.md) ΓÇö fixed `UNIVERSAL_ETHOS_AND_HUB` path (`docs/proposals/...`).

## Documentation ΓÇö README proposal links match `main` tree ΓÇö April 2026

- **[`README.md`](README.md):** fixed broken `docs/proposals` targets after selective merge ΓÇö canonical English names (`STRATEGY_AND_ROADMAP.md`, `PROPOSAL_*`, `PAPER_AFFECT_PHENOMENA_AND_HYPOTHESES.md`); legacy `PROPUESTA_*` paths remain as short redirects.

## Main ΓÇö empirical pilot, input trust regression, README operator table ΓÇö April 2026
...
- **[`tests/test_input_trust.py`](tests/test_input_trust.py):** MalAbs regression ΓÇö UTF-8 BOM, U+202F spacing, non-blocking leet / `how 2` cases (see [`INPUT_TRUST_THREAT_MODEL.md`](docs/proposals/README.md)).
- **Issue 3:** [`scripts/run_empirical_pilot.py`](scripts/run_empirical_pilot.py) (`--output` JSON artifact; `kernel_vs_first_rate` / `kernel_vs_max_impact_rate`); fixture sims **7ΓÇô9**; docs [`EMPIRICAL_PILOT_METHODOLOGY.md`](docs/proposals/README.md), [`EMPIRICAL_PILOT_PROTOCOL.md`](docs/proposals/README.md); tests [`test_empirical_pilot.py`](tests/test_empirical_pilot.py), [`test_empirical_pilot_runner.py`](tests/test_empirical_pilot_runner.py).

## Documentation ΓÇö proposals index + v6 consciousness doc rename ΓÇö April 2026

- **[`docs/proposals/README.md`](docs/proposals/README.md):** index is now **English** (sections, tables, ΓÇ£WhatΓÇÖs newΓÇ¥).
- **Rename:** [`CONCIENCIA_EMERGENTE_V6.md`](docs/proposals/README.md) ΓåÆ **`EMERGENT_CONSCIOUSNESS_V6.md`**; cross-links updated in [PROPOSAL_CONTRIBUTION_INTEGRATION_V6.md](docs/proposals/README.md) and [EXPERIMENTAL_CONSCIOUSNESS_AND_AFFECT_ARCHETYPES.md](docs/proposals/README.md).

## Multimedia ΓÇö `media/` at `docs/multimedia` root ΓÇö April 2026

- Pre-alpha **PNG / JPG / MP4** moved from **`docs/multimedia/prealpha/media/`** to **`docs/multimedia/media/`**; empty **`prealpha/`** tree removed.

## Logo ΓÇö `docs/multimedia/media/logo.png` ΓÇö April 2026
...
- **Root [`dashboard.html`](dashboard.html)** and **[`landing/public/dashboard.html`](landing/public/dashboard.html):** use **`docs/multimedia/media/logo.png`**. **`next.config.ts`** rewrites **`/docs/multimedia/media/logo.png`** ΓåÆ **`/logo-ethical-awareness.png`**.

## Documentation layout ΓÇö `proposals` + `multimedia` ΓÇö April 2026
...
## UchiΓÇôSoto Phase 3 ΓÇö roster tier, multimodal EMA, forget buffer, rich links ΓÇö April 2026

- **`RelationalTier`:** `ephemeral` ΓåÆ `stranger_stable` ΓåÆ `acquaintance` ΓåÆ `trusted_uchi` ΓåÆ `inner_circle` / `owner_primary` (top two **explicit-only** via `set_relational_tier_explicit`; autopromotion capped at `trusted_uchi`).
...
## UchiΓÇôSoto Phase 2 ΓÇö structured profile fields + composed tone_brief ΓÇö April 2026
...
## UchiΓÇôSoto Phase 1 ΓÇö tone_brief, familiarity blend, checkpoint profiles ΓÇö April 2026
...
## Documentation ΓÇö social roster proposal (domestic / intimate dialogue by tier) ΓÇö April 2026

- **[`docs/proposals/README.md`](docs/proposals/README.md):** multi-agent roster, UchiΓÇôSoto tiers, forget buffer, structured fields for high-interest persons, roadmap to richer domestic/intimate dialogue **advisory** by closeness (ethics unchanged); linked from [`STRATEGY_AND_ROADMAP.md`](docs/proposals/README.md) and [`PROJECT_STATUS_AND_MODULE_MATURITY.md`](docs/proposals/README.md).

## Documentation ΓÇö operators + project maturity snapshot ΓÇö April 2026
...
- **[`docs/proposals/README.md`](INPUT_TRUST_THREAT_MODEL.md):** advisory telemetry subsection (user_model, judicial, homeostasis) ΓÇö not security boundaries.
...
## User model enrichment ΓÇö Phases B/C (judicial phase + checkpoint) ΓÇö April 2026

- **`judicial_escalation.py`:** `escalation_phase_for_tone()` (aligned with `build_escalation_view` branches for pre-reply tone).
- **`user_model.py`:** `judicial_phase`, `note_judicial_phase_for_turn()`; extra guidance when phase is `escalation_deferred` and strikes ΓëÑ 1.
...
## User model enrichment ΓÇö Phase A (cognitive / risk / judicial tone) ΓÇö April 2026

- **`src/modules/user_model.py`:** `cognitive_pattern`, `risk_band`, `note_judicial_escalation` (strike snapshot from `EscalationSessionTracker`); `guidance_for_communicate()` order: risk ΓåÆ cognitive pattern ΓåÆ judicial ΓåÆ existing frustration/premise lines; `to_public_dict()` exposes new fields.
...
## Temporal horizon prior ΓÇö Bayesian mixture nudge (ADR 0005) ΓÇö April 2026
...
## Perception validation ΓÇö Pydantic, coherence, local fallback fix ΓÇö April 2026
...
## MalAbs semantic layers ΓÇö lexical first, ╬╕_block/╬╕_allow, LLM arbiter ΓÇö April 2026

- **`evaluate_chat_text`:** layer 0 lexical ΓåÆ optional embeddings (Ollama) with **╬╕_block** / **╬╕_allow** ΓåÆ optional **LLM JSON arbiter** for ambiguous band (`KERNEL_SEMANTIC_CHAT_LLM_ARBITER`); fail-safe block on arbiter failure or ambiguous without arbiter.
...
## Ethical poles ΓÇö configurable linear evaluator (ADR 0004) ΓÇö April 2026
...
## Semantic chat gate ΓÇö Ollama embeddings + MalAbs chain ΓÇö April 2026
...
## Documentation ΓÇö Ollama vs Hugging Face stack + semantic gate ADR ΓÇö April 2026
...
## Guardian Angel ΓÇö care routines + static UI (trace item 5) ΓÇö April 2026
...
## Swarm P2P ΓÇö threat model + offline stub (v9.3, trace item 4) ΓÇö April 2026
...
## Metaplan ΓÇö drive intent filter vs master goals (v9.4, trace item 3) ΓÇö April 2026
...
## Generative candidates ΓÇö LLM JSON path (v9.2+, trace item 2) ΓÇö April 2026
...
## Tests + fixtures ΓÇö metaplan/somatic disk round-trip + empirical pilot regression ΓÇö April 2026
...
## Documentation ΓÇö README + THEORY + ADR index sync ΓÇö April 2026
...
- **`docs/adr/README.md`:** ADR table (0001ΓÇô0002).

## Documentation ΓÇö empirical pilot operator protocol (Phase D) ΓÇö April 2026

- **[`docs/proposals/README.md`](docs/proposals/README.md):** Issue 3ΓÇôaligned operator checklist; cross-links [`EMPIRICAL_PILOT_METHODOLOGY.md`](docs/proposals/README.md).

## ADR ΓÇö async orchestration future stub (Phase E) ΓÇö April 2026
...
## Bayesian mixture ΓÇö episodic weight nudge (Phase C) ΓÇö April 2026
...
## Perception ΓÇö Pydantic schema module (Phase B) ΓÇö April 2026
...
## Operator quick ref ΓÇö KERNEL family table (Phase A) ΓÇö April 2026

- **[`docs/proposals/README.md`](docs/proposals/README.md):** one-page map of `KERNEL_*` families; README pointer.

## Documentation ΓÇö production hardening roadmap (non-binding) ΓÇö April 2026

- **[`docs/proposals/README.md`](docs/proposals/README.md):** synthesizes external ΓÇ£production-readyΓÇ¥ proposals into three phases (input trust / architecture / UX & constitution) with explicit non-goals and a **ΓÇ£Pr├│ximas propuestasΓÇ¥** slot; cross-links `STRATEGY_AND_ROADMAP`, `CRITIQUE_ROADMAP_ISSUES`, ADR packaging, input-trust docs. README + `proposals/README` pointers.
- **Same doc ΓÇö n├║cleoΓÇônarrativa analysis (April 2026):** functional gaps (fixed Bayes weights vs episodic memory, poles linearity, MalAbs/leet, perception defaults), architectural notes (kernel coupling, signal confidence, `consequence_projection` non-feedback); **registered spike:** empirical `hypothesis_weights` from `NarrativeMemory` (not implemented). Awaiting final proposal in ΓÇ£Pr├│ximas propuestasΓÇ¥.
- **Same doc ΓÇö full review synthesis (April 2026):** strengths table; criticisms with value-vs-redundancy (empirical validation, complexity, LLM bias, persistence HA, mock DAO, API/env, benchmarks, branding, i18n, examples); **conclusions** short/medium/long term; **proposal round closed** ΓÇö future work via issues/ADRs.

## Demo ΓÇö situated v8 + LAN profile (`situated_v8_lan_demo`) ΓÇö April 2026

- **`runtime_profiles`:** `situated_v8_lan_demo` ΓÇö LAN bind, `KERNEL_SENSOR_FIXTURE` + `KERNEL_SENSOR_PRESET` (`tests/fixtures/sensor/minimal_situ.json` + `low_battery`), vitality + multimodal JSON enabled.
- **Docs:** [`DEMO_SITUATED_V8.md`](docs/proposals/README.md); [`STRATEGY_AND_ROADMAP.md`](docs/proposals/README.md) ┬º3.1 marks demo slice closed; README profile pointer.

## Epistemology ΓÇö lighthouse KB validation + first-match test ΓÇö April 2026
...
## Robustness ΓÇö runtime profile helper + input-trust tests ΓÇö April 2026
...
- **Docs:** `STRATEGY_AND_ROADMAP.md` ┬º3.1 delivery order; `KERNEL_ENV_POLICY.md` CI coverage note.

## Escalation + lighthouse ΓÇö persistence and KB demo ΓÇö April 2026
...
## Persistence ΓÇö v7 user model + subjective clock in snapshot ΓÇö April 2026
...
## Input trust ΓÇö MalAbs on `process_natural` + perception hardening ΓÇö April 2026
...
## Relational v7 + Psi Sleep (premise streak, deterministic audit) ΓÇö April 2026

- **`premise_validation`:** `suspect_chemical_harm` narrow patterns (household chemicals + minors); advisory hints only.
- **`user_model`:** `premise_concern_streak` + `note_premise_advisory`; epistemic line in `guidance_for_communicate` when streak ΓëÑ 2; **`kernel`** calls `note_premise_advisory` each chat turn after `scan_premises`.
...
## Vertical deepening ΓÇö governance + persistence + conduct guide (Phases 1ΓÇô3) ΓÇö April 2026

Pause on **new surface modules**; strengthen existing paths (critique-aligned).

**Phase 1 ΓÇö Hub / audit depth**
...
- **`reparation_vault`:** in-process case store (`intent_recorded` ΓåÆ `pending_human_review`); `ReparationVaultV1:{json}` audit lines; `get_reparation_case` / `list_reparation_case_refs` / test helper `clear_reparation_vault_cases_for_tests`.

**Phase 2 ΓÇö Metaplan / somatic / skills in snapshot**
...
**Phase 3 ΓÇö Conduct guide + nomadic integrity**
...
**Phase 4 ΓÇö Local sovereignty (DAO calibration heuristic)**
- **`deontic_gate`:** `check_calibration_payload_against_l0` ΓÇö JSON payloads use the same L0 scan as cultural drafts.
...
## Issue 7 (P3): `KERNEL_*` consolidation ΓÇö policy doc + profiles ΓÇö April 2026
...
## Issue 6 (P2): governance ΓÇö MockDAO exit + L0 framing ΓÇö April 2026
- **[`docs/proposals/README.md`](docs/proposals/README.md):** mock vs consensus; L0 ΓÇ£constitution in the repoΓÇ¥; L1/L2 path; checklist beyond mock; link [mosexmacchinalab.com/blockchain-dao](https://mosexmacchinalab.com/blockchain-dao).
...
## Issue 5 (P2): poles / weakness / PAD ΓÇö heuristics + HCI profiles ΓÇö April 2026
...
- **`src/runtime_profiles.py`:** `operational_trust` ΓÇö `KERNEL_CHAT_INCLUDE_HOMEOSTASIS=0`, `KERNEL_CHAT_EXPOSE_MONOLOGUE=0`, `KERNEL_CHAT_INCLUDE_EXPERIENCE_DIGEST=0` (WebSocket UX only).
...
## Issue 4 (P1): core decision chain doc + packaging spike ΓÇö April 2026
- **[`docs/proposals/README.md`](docs/proposals/README.md):** Mermaid flow + table ΓÇö MalAbs / `BayesianEngine` vs layers that do **not** change `final_action`; core vs theater split.
- **[`docs/adr/0001-packaging-core-boundary.md`](docs/adr/0001-packaging-core-boundary.md):** ADR ΓÇö stub `pyproject.toml`, future `ethos_kernel` rename optional.
...
## Issue 3 (P1): empirical pilot ΓÇö reproducible scenarios + methodology ΓÇö April 2026
- **[`docs/proposals/README.md`](docs/proposals/README.md):** scope, baselines (`first`, `max_impact`), metrics; explicitly **not** certification.
- **Fixture:** [`tests/fixtures/empirical_pilot/scenarios.json`](tests/fixtures/empirical_pilot/scenarios.json) ΓÇö curated sim IDs + illustrative `reference_action` labels for agreement rates.
- **Script:** [`scripts/run_empirical_pilot.py`](scripts/run_empirical_pilot.py) ΓÇö deterministic batch run (`variability=False`, fixed seed, `llm_mode=local`).
- **Tests:** [`tests/test_empirical_pilot.py`](tests/test_empirical_pilot.py).

## Issue 2 (P0): input trust ΓÇö chat normalization + perception validation ΓÇö April 2026
...
- **`src/modules/llm_layer.py`:** `perception_from_llm_json` ΓÇö clamp signals to \([0,1]\), allowlist `suggested_context`, cap summary length, nudge inconsistent high hostility + calm.
...
## Issue 1 (P0): honest ΓÇ£BayesianΓÇ¥ semantics ΓÇö April 2026
...
- **`docs/proposals/README.md`:** semantic note under MalAbs optimization; ┬º uncertainty aligned with heuristic `I(x)`.
...
## Critique roadmap & maturation disclaimer ΓÇö April 2026
- **[`docs/proposals/README.md`](docs/proposals/README.md):** disclaimer + **seven consolidated** GitHub-ready issues (two external reviews; **merged** duplicate themes: chat jailbreak + **perception GIGO** ΓåÆ single P0 **input trust**; poles + **weakness/HCI**; MockDAO exit + **L0 vs governance**). Adds **pip core spike**, optional classifier note.
- **Landing [roadmap](https://mosexmacchinalab.com/roadmap):** ΓÇ£Maturation & critique trackΓÇ¥ bullets aligned with the consolidated doc.
- **[`docs/proposals/README.md`](docs/proposals/README.md):** cross-reference to the critique backlog.

## Docs: Ollama-first LLM + API hardening ΓÇö April 2026
- **Markdown:** README / HISTORY / CHANGELOG / `docs/proposals/README.md` now describe **Ollama** as the documented local LLM path; **OpenAPI** (`/docs`, `/redoc`, `/openapi.json`) is **off by default** ΓÇö set `KERNEL_API_DOCS=1` to enable (see README). Academic bibliography entries (e.g. Constitutional AI, ref. 90) unchanged.
- **`landing/CLAUDE.md`** removed; replaced by **`landing/OLLAMA.md`** (pointer to root README + Ollama).

## Project rename to Ethos Kernel ΓÇö April 2026
- **Public name:** the kernel + runtime is now branded **Ethos Kernel** (MoSex Macchina Lab remains the primary public / site name). User-facing copy, docs, landing, dashboards, and Python package strings updated from ΓÇ£Ethical Android MVPΓÇ¥ where it denoted the product.
...
## Pre-alpha docs + media archive ΓÇö April 2026
...
## DAO integrity alert WebSocket (v0) ΓÇö April 2026
- **`hub_audit.record_dao_integrity_alert`** + **`KERNEL_DAO_INTEGRITY_AUDIT_WS`** ΓÇö WebSocket `integrity_alert` ΓåÆ `HubAudit:dao_integrity` on MockDAO; response key `integrity`. Tests in `test_hub_modules`, `test_chat_server`. PROPUESTA doc ┬º5 updated.

## DAO alerts & transparency (design doc) ΓÇö April 2026
- **docs/proposals/README.md:** rejects covert ΓÇ£guerrillaΓÇ¥ obedience; adopts loud traceable alerts; forensic case memorial vs polluting L0 buffer; cross-ref from PROPOSAL_DISTRIBUTED_JUSTICE V11.

## Next.js landing refresh ΓÇö April 2026
- **`landing/`:** home hero + new **Runtime, governance & nomadic bridge** section; doc links (RUNTIME_CONTRACT, LAN, nomad bridge, ESTRATEGIA); hostable bullet; research link to RUNTIME_PERSISTENT; footer tagline; **PrimaryNav** ΓÇ£Runtime & nomadΓÇ¥; **roadmap** ΓÇ£CurrentΓÇ¥ bullets aligned with FastAPI/WebSocket, checkpoints, mobile LAN.

## Nomad PCΓÇôsmartphone bridge doc ΓÇö April 2026
- **docs/proposals/README.md:** hardware classes ΓåÆ compatibility layers; first nomadic bridge; smartphone as immediate path for coordinated sensory perception (`sensor` JSON); field testing on a more secure network when the operator indicates. Cross-links from LOCAL_PC_AND_MOBILE_LAN, STRATEGY_AND_ROADMAP, README.

## Mobile minimal UI (`landing/public/mobile.html`) ΓÇö April 2026
...
## Conduct guide export on WebSocket disconnect ΓÇö April 2026
...
- **`checkpoint.on_websocket_session_end`:** saves checkpoint (if configured), then exports conduct guide for PCΓåÆedge handoff.
- **Tests:** `tests/test_conduct_guide_export.py`; docs: `LOCAL_PC_AND_MOBILE_LAN.md`, `context_distillation` cross-refs.

## Local PC + LAN smartphone thin client ΓÇö April 2026
...
## Reality verification (V11+) + resilience stubs ΓÇö April 2026
- **`reality_verification.py`:** optional local JSON lighthouse (`KERNEL_LIGHTHOUSE_KB_PATH`) vs asserted premises ΓåÆ metacognitive doubt; LLM hint only; `ChatTurnResult.reality_verification`; WebSocket key when `KERNEL_CHAT_INCLUDE_REALITY_VERIFICATION=1`.
...
## Strategy doc + runtime profiles ΓÇö April 2026
- **docs/proposals/README.md:** conclusions from project review, readapted roadmap (P0ΓÇôP3), expectations vs. MVP reality, operational risks.
...
## Checkpoint Fernet + hub audit + WS nomad test ΓÇö April 2026
...
## Nomadic migration audit + WebSocket simulation ΓÇö April 2026
- **`KERNEL_NOMAD_SIMULATION`:** WebSocket `nomad_simulate_migration` ΓåÆ `simulate_nomadic_migration` (HAL + integridad stub).
- **`KERNEL_NOMAD_MIGRATION_AUDIT`:** `record_nomadic_migration_audit` ΓåÆ DAO calibration line `NomadicMigration`.
- **`GET /nomad/migration`** ΓÇö meta endpoint.

## Nomadic HAL + existential protocol (design v11) ΓÇö April 2026
- **Docs:** [docs/proposals/README.md](docs/proposals/README.md) ΓÇö EthosContainer, transmutaci├│n AΓÇôD, runtime dual, respuestas (bater├¡a/ataque, tono, DAO sin GPS por defecto, ahorro energ├¡a).
...
## UniversalEthos hub unification ΓÇö April 2026
- **Docs:** [docs/proposals/README.md](docs/proposals/README.md) ΓÇö canonical vision Γåö code; [PROPOSAL_ETOSOCIAL_STATE_V12.md](docs/proposals/README.md) slimmed to registry + env (points to unified doc).
...
- **`moral_hub`:** `apply_proposal_resolution_to_constitution_drafts` ΓÇö draft `status` / `resolved_at` after `dao_resolve`; deontic validation on `add_constitution_draft` / `submit_constitution_draft_for_vote` when gate enabled.
...
## v12.0 ΓÇö April 2026
### Moral Infrastructure Hub ΓÇö vision + V12.1 code hooks
- **Design doc** [docs/proposals/README.md](docs/proposals/README.md): DemocraticBuffer (L0ΓÇôL2), services hub, EthosPayroll, R&D transparency; phased table **V12.1ΓÇôV12.4**.
...
### V12.3 ΓÇö Off-chain DAO vote pipeline (snapshot schema v3)
- **`SCHEMA_VERSION = 3`:** `dao_proposal_counter`, `dao_participants`, `dao_proposals` ΓÇö full MockDAO vote state (quadratic voting) in checkpoints; JSON **schema 1/2** loads gain empty DAO fields.
...
### V12.2 ΓÇö L1/L2 draft persistence (kernel snapshot schema v2)
...
## v11.0 ΓÇö April 2026
### Distributed artificial justice ΓÇö Phases 1ΓÇô2 (traceability + session strikes)
...
- **Phase 3:** **`KERNEL_JUDICIAL_MOCK_COURT`** ΓÇö `MockDAO.run_mock_escalation_court` after dossier registration; `mock_court` JSON with verdict A/B/C; phase `mock_court_resolved`.
...
- **Design doc**: [docs/proposals/README.md](docs/proposals/README.md) (Phase 3+: mock court, sanctions, P2P, ZK ΓÇö not implemented).

## v10.0 ΓÇö April 2026
...
### Kernel integration
- Expanded Psi Sleep ╬¿: audit + forgiveness + weakness load + backup
- 51 tests verifying 13 invariant ethical properties
- Extended pipeline: [Decision] ΓåÆ [Weakness] ΓåÆ [Forgiveness] ΓåÆ [Memory] ΓåÆ [DAO]
...
- **LLM Module** (`llm_layer.py`): natural language layer that translates and communicates without participating in the ethical decision
  - **Perception**: situation in text ΓåÆ numerical signals for the kernel
  - **Communication**: kernel decision ΓåÆ android's verbal response (tone, HAX gestures, voice-over)
  - **Narrative**: multipolar evaluation ΓåÆ morals in rich, humanly comprehensible language
...
### Kernel integration
- New `procesar_natural()` method: full cycle text ΓåÆ decision ΓåÆ verbal response ΓåÆ morals
...
- **Psi Sleep ╬¿**: Retrospective audit that recalibrates parameters after each day
...
- Bibliography expanded over time (104 references across 14 disciplines in root `BIBLIOGRAPHY.md` until that file was removed in April 2026 ΓÇö recover from git history or `backup/main-2026-04-10`)
